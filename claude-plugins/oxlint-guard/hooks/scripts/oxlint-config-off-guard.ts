...
export function detectConfigWeakening({
    ...
    for (const re of OFF_DIRECTIVE_PATTERNS) {
      if (re.test(newString) && !re.test(oldString)) {
        return {
          weaken: true,
          reason:
            `Patch adds a config-weakening directive (matched ${re.source}) to ${filePath}. The constitution forbids weakening the oxlint config.`,
        }
      }
    }
  ...
  if (TS_MODULE_RE.test(filePath)) {
    if (!/\bdefineConfig\b/.test(content)) {
      return {
        weaken: true,
        reason: `${filePath} does not declare an oxlint \`defineConfig(...)` +
    ...
    for (const re of OFF_DIRECTIVE_PATTERNS) {
      if (re.test(content)) {
        return {
          weaken: true,
          reason:
            `${filePath} contains a config-weakening directive (matched ${re.source}). The constitution forbids weakening the oxlint config.`,
        }
      }
    }
    return { weaken: false, reason: '' }
  ...
  if (JSON_CONFIG_RE.test(filePath)) {
    ...
    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
      return {
        weaken: true,
        reason: `${filePath} must be a JSON object (the oxlint config shape), not ${
      ...
      for (const value of Object.values(rules as Record<string, unknown>)) {
        if (value === 'off') {
          return {
            weaken: true,
            reason:
              `${filePath} disables at least one rule (\`"off"\`). The constitution forbids weakening the oxlint config.`,
          }
        }
      }
    }
