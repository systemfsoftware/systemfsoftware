...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Locked**           | `AGENTS.md`, `repos/`, `.github/workflows/`, evaluation gates (`packages/stryker-js/core/src/reporters/test-contribution.ts`, `scripts/guard-mutate-scope.mjs`, `scripts/check-lint-coverage.mjs`, `scripts/check-stryker-config.mjs`) | Read and propose changes, but do not edit to make verification pass.                                                                                                                                                                                                                      |
...
```bash
pnpm check  # pnpm install --frozen-lockfile → turbo (format:check, lint, typecheck, test, attw, api:check) → check:exports → check:mutate-scope → check:lint-coverage → check:no-hand-rolled-jsonc → check:publish-config → check:project-references → check:stryker-config
```
