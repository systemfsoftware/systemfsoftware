...
- **REPO-D3** — behaviour-bearing work reaches a pull request only through the lifecycle: plan, implement, review, fix, then open it. The session's first PR-opening tool stream is interrupted before the call runs and the lifecycle is injected; re-issuing the call proceeds, so this binds by review, not by refusal. The rule reads no diff — whether a change is trivial is the author's judgement, stated in the PR. Gate: `.omp/rules/pr-lifecycle.md`, an OMP TTSR rule scoped to tool streams and firing once per session on `pr_create` or `gh pr create`; `omp ttsr test '{"op":"pr_create"}' --source tool --tool write` reproduces the match.
