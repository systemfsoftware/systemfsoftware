...
- **REPO-R2** — an intent is required only for a change that reaches consumers: a file matching the `build` task's `inputs` inside a publishable package, or a consumer-observable manifest field. A test config, a fixture or local tooling reaches nobody, so it ships with no `.changeset/` file at all — an unrequested intent is a file review must open and dismiss. When one is required, `pnpm change --bump <none|patch|minor|major>` takes the bump from what a consumer observes from outside the package, never from the diff's size: `none` is exactly the case where shipped sources changed but no exported name, type or behaviour did, and a body that needs the words "unchanged public surface" to be accurate has already named its own bump. Gate: `scripts/guards/check-changeset.ts` decides whether an intent is required, never which bump is right.
