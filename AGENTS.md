...
| **REPO-T1** | Hand-written in-source tests are authored inline snapshots over non-exported symbols; hand-written property constructs and every other assertion channel are banned in-source; the generated schema-law channel (`ruleOfSchemas`) is exempt. | `pnpm check:local` — both rules ride `@systemfsoftware/oxlint-plugin-effect-dmmf`'s recommended set through `@systemfsoftware/oxlint-config/base` at error; on overlap, the snapshot-only message is the binding constraint |
