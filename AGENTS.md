...
- **REPO-W9** — every settled finding is written to persistent memory at the moment it settles, never batched at session end. Before re-deriving a question this session or a prior one may have settled, recall from the harness's persistent-memory facility first; when a finding settles, store it there with the subject, the evidence that decided it, and the observation that would reverse it. Taxonomy findings additionally enter `references/ledger.md` under REPO-W5's protocol; `docs/solutions/` records solved problems per REPO-E1. A finding that exists only in conversation is not retained — the next fresh context re-derives it and the review cannot see it. Gate: review — the reviewer names the memory record or ledger entry each settled finding of the session lives in; a settled finding with no durable record is a failure of this rule.
- **REPO-W10** — this repository is under constant grilling: panels of adversarial and correctness agents argue design questions over IRC while construction proceeds, and you are the adjudicator, never a debater. Deriving means the agents argue with each other — a position you reason through alone is a hypothesis, and a panel that agrees is one perspective sampled N times. Dispatch panels framed as neutral questions (never with the expected verdict in the task text), mandate the open/attack/answer/converge rounds explicitly, admit findings only with an executable reproduction observed this session, and strike any argument whose warrant is precedent — the repository is the subject under test, never the evidence for its own correctness (REPO-W7). A settled question that lives only in the panel transcript is re-litigated by the next fresh context; route every verdict to the ledger or a decision record. Gate: review — for each settled design question the reviewer names the panel that derived it and the disagreement it resolved, or states that no panel ran.
- **REPO-W11** — the sync schema API (`S.decodeSync`, `S.encodeSync`, `S.parseSync`, `S.assertsSync`) appears only in tests and throwaway scripts, never in production source. Production code decodes and encodes through the Effect channel: `S.decode(...)` / `S.encode(...)` piped to `Effect.orDie` for a trusted literal whose error channel is genuinely never inhabited, or `Effect.mapError` when the failure is a real outcome. A `*Sync` call throws an untyped exception outside the error channel, so a caller cannot handle it and the type system loses the failure. Gate: `grep -rn "decodeSync\|encodeSync\|parseSync\|assertsSync" npm/apps/terok --include="*.ts" | grep -v "__tests__" | grep -v "scripts/"` returns nothing.
