...
| `docs/solutions/`                   | researching past decisions, postmortems, or known patterns (categorized with YAML frontmatter: module, tags, problem_type)                      |
...
| `apps/`           | Rust binary crates built by the root Cargo workspace           | Root invariants                          |
| `crates/`         | Rust library crates built by the root Cargo workspace          | Root invariants                          |
| `packs/`          | Rule packs, each rule beside its fixtures, embedded at build   | Root invariants                          |
| `npm/`            | npm launcher packages for the Rust binaries (`npm/<name>`)     | Root invariants; `REPO-S5`               |
...
| **REPO-S5** | A package lives at `packages/<family>/<name>`, `packages/<name>`, or, for the npm launcher of a Rust binary built in this repository, `npm/<name>`, and `<name>` is its unscoped npm name. A family folder holds ≥2 packages that share one named concept and build on an anchor inside it; a package consumed by more than one family, or with no family, stays at `packages/<name>`. Never add a grab-bag folder, and never name a family folder after a directory segment a path-matching lint rule treats specially (`RAW_VITEST_PACKAGES` in `oxlint-plugin-test-discipline`). | `review`                                                                                    |
