...
Review the entire application and every cross-layer relationship through the literal **review loop until dry**. Every round starts by reading every requirement under `docs/analysis/` in full, then reads the complete scope against it. The round gates are clean backend `pnpm check:watch`, backend `pnpm dev`, and frontend `pnpm dev` processes.

Read `AGENTS.md`, `.agents/skills/review/SKILL.md`, and every detailed procedure under `.agents/skills/review/` in full before reviewing, and follow them without any discretionary change.
