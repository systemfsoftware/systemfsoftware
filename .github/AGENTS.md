...
- **Mutation runs on main only, sharded.** `mutation.yml` triggers on a push to main and on `workflow_dispatch`. Pull requests never run mutation. Every shard runs `turbo mutation` over each package with a `mutation` script. `STRYKER_SHARD=<k>/<n>` makes that package's `shardMutate` (`@systemfsoftware/stryker-config`) keep one disjoint slice of its files, and each shard keeps its own incremental cache.
