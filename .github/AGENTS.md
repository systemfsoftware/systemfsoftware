...
- **`check:ci` is the definition of the gate.** `package.json#scripts.check:ci` defines the container-free gate; `.github/workflows/reusable-checks.yml` invokes it. CI workflows enumerate no check steps. Gate: `pnpm check:ci` exit code.
- **Each CI lane owns its turbo cache key prefix** (`turbo-<os>-checks-<sha>`, `turbo-<os>-release-<sha>`). actions/cache keys are immutable: lanes sharing a primary key race, and the faster lane's partial save silently drops the slower lane's entries.
- **`Release` is push-triggered; phase is derived from repository state** by `scripts/tools/plan-release.mjs` (`phase=`):
  1. `publish` — some `name@v<version>` tag is absent from `origin`. Runs the gate, builds, publishes via npm OIDC trusted publishing, tags, creates GitHub Releases.
