...
| `lint:json` fails (conventions findings)        | A first-party `tsconfig.json` lost its `references: [{ "path": "./tsconfig.node.json" }]` entry | Add the entry (or a sibling `tsconfig.node.json`) — exemptions are listed in root `AGENTS.md`                  |
| `lint:json` fails (`no grit engine found`)      | The grit engine is not on PATH                                                                  | The flake devShell (or CI's `.#grit` build) provides it; the npm postinstall is deliberately denied            |
