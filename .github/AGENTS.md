...
- **Never re-trigger a release by hand.** `scripts/tools/plan-release.ts` derives the phase from registry state, so a killed publish leaves 404s the next push re-plans. npm's 409 "previously staged version" / 403 "previously published versions" means `held`, not failed (`publishOutcome` in `scripts/tools/publish-set.ts`).
- **CI cannot debut a package** (OIDC cannot publish a first version): a maintainer runs `pnpm publish:unpublished` and registers the trusted publisher. gritlint's six names are the exception: `./scripts/tools/gritlint/bootstrap-npm.ts` debuts them, and `release.yml`'s `gritlint-publish` gate (`bootstrap-npm.ts --check`) fails the release while one is missing.
