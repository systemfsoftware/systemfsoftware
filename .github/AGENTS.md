...
| `guard:projects` fails (`CONFORMANCE-ENROLLMENT`) | A package whose source forks fibers, holds Queue/Deferred/Ref/Semaphore state, or acquires scoped resources does not run vitest through the shared defineConfig | Give it a vitest test script and a vitest config using defineConfig from @systemfsoftware/vitest-config; its run then reports each primitive site no conformance check exercises |
