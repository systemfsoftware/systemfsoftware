...
- **The publish step publishes the captured release set, never a set pnpm re-derives.** `scripts/tools/tag-released-packages.ts --publish` filters the recursive publish to each captured member and settles its verdict against the registry (`scripts/tools/publish-set.ts`, `scripts/tools/publish-set.test.ts`). pnpm's own published-check reads a packument the registry caches for five minutes, so a recursive publish left to it re-PUTs a version npm already serves and dies on that 403.
