...
Resolving that import needs `moduleResolution` set to `bundler`, `node16` or `nodenext`. The legacy `node` setting reads no entry-point map, so it cannot see the second entry; on that setting the refusal surface is unreachable.
