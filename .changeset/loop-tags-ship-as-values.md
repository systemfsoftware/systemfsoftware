...
`PollLoopTag`, `StreamLoopTag` and `SubscriptionLoopTag` are new exports: each is the frozen
`{ _tag }` object together with a type of the same name, and `PollLoop`, `StreamLoop` and
