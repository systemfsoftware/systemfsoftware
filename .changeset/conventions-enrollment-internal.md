...
Internal only: the JSON conventions enrollment regenerated the lockfile (additive — the conventions package, its exact-pinned grit engine dependency, effect dev-dependencies) and enrolled per-package node tsconfigs; no consumer-observable change in any listed package.
