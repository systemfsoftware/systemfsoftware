...
- The subpath entries `Run`, `Module`, `Metrics`, `ReporterEvent`, `output-file` and `provided-options` are gone. Their types now live on the abstraction that owns them — the run-event stream is internal to the host, `ReporterEvent` is part of `Reporter`, metrics are part of `Report`, and the option helpers are part of `Options`.
