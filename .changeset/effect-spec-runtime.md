...
The error type and the case-body type are parameters of the suite, so a consumer declares its own error union and case shape. `TaskRef` hands the running test's task to the case body, and `SuiteScope` resolves a scope map into the services it names.
