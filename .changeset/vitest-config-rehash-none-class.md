...
Turbo re-hashes every publishable package when a vitest config file changes anywhere in the tree, so this intent records the class the gate itself named: none of these packages changed anything a consumer observes. The only behavioral surface is the stryker CLI contract lane, covered by its own intent
