...
- **Backend Stack: Effect-v4-native RPC + drizzle + better-auth** (redirect-settled):
  ...
  - Persistence: `drizzle-orm@1.0.0-rc.4` (latest clean RC; `rc` dist-tag; peers `effect >=4.0.0-beta.83`, v4-native) via `drizzle-orm/effect-postgres` over `@effect/sql-pg@4.0.0-rc.116` (rc dist-tag matches the repo pin exactly), with `drizzle-kit` for migration generation. Drizzle's `createInsertSchema` effect-schema codegen is v3-oriented and is NOT used: domain contracts stay hand-authored `S.Class`, rows decode via `S.decodeUnknown` at the port.
...
- **Version Pinning: catalog-exact, no floating aliases**: `drizzle-orm: 1.0.0-rc.4` pinned exactly (an rc.5 canary `1.0.0-rc.5-5935859` exists; when a clean rc.5 lands, the bump is a one-line catalog change). Effect-line deps ride the existing `4.0.0-rc.116` pin. No `@rc`/`@next` alias resolution anywhere.
...
| `drizzle-orm`           | `1.0.0-rc.4` exact                                             | Latest clean RC (`rc` dist-tag; `rc5` tag is a commit-suffixed canary). Peers `effect >=4.0.0-beta.83` (optional, v4-native). Ships `effect-postgres` and `effect-pglite` export trees (driver/session/codecs/migrator). dev-targets `effect 4.0.0-beta.83`. |
| `drizzle-kit`           | rc line, exact at install time                                 | Generates SQL migrations from the drizzle schema.                                                                                                                                                                                                            |
| `better-auth`           | exact version validated by the U0 spike, pinned in the catalog | Official Drizzle adapter; framework-agnostic request handler; `auth.api.getSession({ headers })` for RPC guard. Peer compatibility with drizzle rc validated by the Phase 0 spike.                                                                           |
...
- **Modified**: `pnpm-workspace.yaml` — add `examples/*` to the packages list; add catalog entries: `drizzle-orm: 1.0.0-rc.4`, `drizzle-kit`, `@effect/sql-pg: 4.0.0-rc.116`, `@effect/sql-pglite: 4.0.0-rc.116`, `better-auth` (exact spike-validated version, never a floating tag), `@electric-sql/pglite`.
...
| ---------------------------------------------------------------------------- | ---------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| drizzle (dev-targets effect `4.0.0-beta.83`) drifts from `4.0.0-rc.116` APIs | Low        | Blocks U5–U9   | U0 spike retires before any code unit; fallback pin `1.0.0-rc.5-5935859` canary exists; catalog-exact pins prevent silent drift.                                                              |
...
| better-auth ↔ drizzle-orm@1.0.0-rc.4 adapter mismatch                        | Low        | Blocks U7 auth | U0 spike roundtrips signup + getSession; fallback is better-auth's generic adapter config or a version pairing recorded in the U10 decision doc.                                              |
