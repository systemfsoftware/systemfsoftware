...
**Files.** `scripts/tools/check-npm-publish.ts` (replaces the `.sh`), `scripts/tools/tag-released-packages.mjs`, `scripts/tools/plan-release.mjs`, `.github/workflows/release.yml`.

**Approach.** The bash script was the outlier in `scripts/tools/` — every sibling is Deno with declared permission scopes, and it needed undeclared `jq` and `curl` on PATH, which is why it could not run locally without a `nix shell` wrapper. Port it to Deno: a pure `classify(localVersion, snapshot)` carrying the whole five-class verdict, native `fetch` with a per-request deadline the caller classifies as `error`, bounded concurrency, and a `--selftest` over the classifier. Then partition the release rather than blocking it. Two emitted spellings of the deferred set: `--emit-filters` for `pnpm publish -r`, `--emit-deferred` for the steps that must not touch it. `tag-released-packages.mjs` gains `--exclude <file>`, so a package npm never received gets neither a tag nor a GitHub Release. `plan-release.mjs` gains `--deferred <file>`, forwarding it to the tag script rather than re-deriving the set, so an unreleasable package cannot pin the phase at `publish` forever; it annotates each one instead. Because the planner now writes annotations to stdout, its key=value block moves to `--output <file>` — appending stdout to `$GITHUB_OUTPUT` would write annotations in as bogus keys. Preserve the `--no-git-checks` comment verbatim: it explains a flag no other job may copy.

**Test scenarios.** `classify` is pure, so the selftest drives the same function the run drives: one case per class, plus `error` outranking a present attestation (a network fault must never read as `ok`). The emitted `--filter` spelling is asserted for the empty, one-package and two-package cases. The tag script's selftest asserts that an excluded package is dropped from the cycle and a non-excluded one is kept.

**Verification.** The port must reproduce the bash verdict exactly. Run both against the live registry and compare the class counts against run `32650470424`'s `unpublished: 1, no-oidc: 1, stuck: 32, ok: 3`. Confirm exactly one exclusion, for `@systemfsoftware/stryker-test-contribution`; confirm the planner's owed count drops by one with `--deferred` supplied; confirm the output file receives only key=value lines; confirm no `jq`, `curl`, `mapfile` or `bash` invocation survives in the port.
