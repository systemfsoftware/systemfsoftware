...
- Wiki rulings, restated because the corpus does not ship with the clone. Each is cited for the decision it settles, with the strength of its ground named, because a ruling grounded in a captured source outranks one that is a local pick among options the sources leave open.
  ...
  - **Nil result, recorded so it can be falsified.** The corpus does not rule on streaming, event transport, or presentation architecture. Four `wiki`-scoped BM25 queries on 2026-08-07 - `"Stream effect primitive presentation"`, `"terminal rendering shell boundary"`, `"event stream library consumer render"`, `"effects are values lazy description"` - returned only `Log`, `First Principles`, `The Axes of Program Units`, and `Inert Composition Value`. KTD9 therefore rests on `CONSTITUTION.md` plus external evidence, not on a wiki ruling. Anyone holding the wiki can re-run those queries and falsify this.
