...
- KTD8. **No property test ships for either detector, and the example rows are not one-per-regex.** A first pass wrote a property test per kernel and both were deleted: their generators were `fc.constantFrom` over a hand-picked list of phrases, which is `architect-property-tests` PT9 — an enumerated sample of an open domain, scenario testing wearing a generator — and their two strongest-looking invariants (`≡SameNotice`, casing-equivalence) both survive a constant-returning implementation, which is PT4. A detector that is a table of regexes has no universal invariant that is not a restatement of the table or a tautology. Review then proposed the opposite extreme: one row per regex, 60 rows, to close the "40+ patterns unexercised" gap. Rejected. A row that exists so a regex is named somewhere is a coverage statistic, not a test — it restates the table in a second file and goes red only when someone edits the very line it copies, which is the tautology PT4 already bans. What earns a row is a _behavioral rule_: each weight class, the stacking rule, the caps boost, each quoted-span stripping strategy, silence on ordinary requests, and the two notice texts byte-for-byte. The suite is sized to those rules, and the deletion test is what proves it: dropping `THRESHOLD` from 2 to 3 and removing one correction regex turned 29 of the then-46 cases red. Review did find one real hole by the same standard — the single-quote branch of `withoutQuotedSpans` was a stripping strategy with no row, and deleting it left all 46 green. That earned the 47th row, and re-running the mutant confirmed the row kills it.
- KTD9. **Detection tuning is out of scope and deferred, not accepted.** Review filed two blockers on false-positive cost: at `THRESHOLD = 2` any single weight-2 category fires, so ordinary professional prompts ("that is incorrect", "we don't need that", "let's move on") trip the full intervention; and `/again\?/i` fires on any question containing "again?" with no surrounding context. Both are faithfully ported behavior (R6, R8) and both reproduce in WireBlast. Retuning the threshold, narrowing a pattern, or gating the two hooks so one prompt cannot trip both at once changes what the hooks _decide_, which is a product judgment about how loud this guard should be — not a port defect. The port is the deliverable; the tuning is a follow-up that needs a human to set the taste.
...
- `pnpm --filter @systemfsoftware/claude-correction-plugin test` — both Gherkin suites green (47 cases).
