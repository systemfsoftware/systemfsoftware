...
- **`packages/oxlint-plugins/recommended/scripts/guard-no-behavior.mjs`.** U6 reorders its `stripLiterals` so string literals are replaced _before_ comments are stripped, fixing a live false-negative in a gate. **This is outside the user's original ask** and is included only because U6's repo-wide scan forces the file into scope: it needs an exemption entry, and an exemption that parks a live defect is the §V.6 smell. The exemption's reason is that it strips comments from **JavaScript source**, not JSONC, so `@std/jsonc` is not its remedy; migrating it to the `oxc-parser` already in the workspace is the proper fix and is deferred. That package forbids test scripts by its own design (`forbiddenScripts` includes `test`), so the reorder ships uncovered — stated, not hidden.
...
**Non-goals.** Resolving `references` through `extends` chains (upstream does not either). Adding a stryker mutation config to either `stryker-js` package. Enrolling `packages/stryker-js/` in per-package oxlint coverage — see KTD-7. Bundling `@std/jsonc` into the published artifacts — see KTD-1.
...
**KTD-1 — Depend on `@std/jsonc` via the workspace catalog's `jsr:` protocol; accept the consumer cost explicitly.**
...
_The consumer cost, accepted with the failure mode named._ Both stryker packages publish to public npm (`core` carries `publishConfig.access: public`; neither is private). Adding `@std/jsonc` to runtime `dependencies` makes every downstream consumer resolve `@jsr/std__jsonc` and its compat chain from `npm.jsr.io` — a second registry host. **Installs behind a single-upstream mirror, an allowlist proxy, or an air-gap will fail**, and that support load lands on this team. Bundling the parser into `dist` (tsdown `noExternal`) would hide it from consumers entirely and is technically clean — the emitted `parse.js` has zero runtime imports. It is **rejected** because the repo has no `noExternal` anywhere and neither package has a `tsdown.config.ts` at all, so bundling means introducing a novel build behavior across a vendored fork to serve a constraint no requirement in this repo states. Revisit if a consumer reports a mirrored-registry failure.
...
| The guard trips on `guard-no-behavior.mjs`, which legitimately strips comments from JavaScript.                                   | One exemption entry carrying its reason inline, plus U6 reordering that file so the exemption is not parking a live defect (§V.6).                                                                                                            |
...
**Files.** `pnpm-workspace.yaml`; `packages/stryker-js/typescript-checker/package.json`; `packages/stryker-js/core/package.json`.

**Approach.** Mirror line 30's `"@std/toml": jsr:^1.0.11` — add `"@std/jsonc": jsr:^1.0.2` to the same `catalog:` block, then `"@std/jsonc": "catalog:"` in each package's `dependencies` (runtime, not dev — both ship it). Do not touch `minimumReleaseAgeExclude` (REPO-S2). Do not hand-edit `exports` (REPO-S4).
...
- `packages/stryker-js/typescript-checker/src/tsconfig-helpers.ts` — delete lines 61-65; rewire `parseConfigFileTextToJson` (67-74); fix the cast at line 92.
...
**Approach.** `parseConfigFileTextToJson` keeps its `{ config } | { error }` shape — both call sites branch on it and `determineBuildModeEnabled`'s contract is unchanged. Inside: strip a leading `\uFEFF` (KTD-4), then `parse` inside the existing try/catch. `parse` throws `SyntaxError`, which the existing catch already narrows to `Error`. The `fileName` parameter stays unused, as today — it mirrors the removed upstream signature; do not repurpose it. For the cast (KTD-9): guard that the parsed value is a non-null, non-array object before reading `compilerOptions` or spreading it; a non-object config contributes no compiler options.
...
- A genuinely malformed document (`{ "a": }`) returns `{ error }` with `config` undefined.
- **Truncated input (`{` and `{"a":`) returns `{ error }`** — pinning that the empty-input class stays an error rather than silently becoming `{}`. This is the guard against the cut R5 sneaking back in as an over-accept.
- An unterminated block comment returns `{ error }` — pinning the KTD-3 residual as _known and deliberate_ rather than accidental.
- Covers AE7 / R10 — `overrideOptions` given a config that is a bare string, and given one that is an array, returns a well-formed object with the override compiler options and no character-indexed keys.
...
- `packages/stryker-js/core/src/sandbox/parse-config-helper.ts` — delete `stripJsonComments` including its JSDoc (lines 8-68); rewire `parseConfigFileTextToJson` (82-89).
...
- Rewrite the four subject-less assertions as `parseConfigFileTextToJson` assertions over the same inputs, asserting the parsed value rather than intermediate text.
...
- `packages/oxlint-plugins/recommended/scripts/guard-no-behavior.mjs` — reorder `stripLiterals` (lines 25-31) so string and template literals are replaced _before_ comments are stripped. Flagged in Scope Boundaries as outside the original ask, included because the exemption would otherwise park a live defect.
...
- [ ] Every guard exemption carries its reason in the source; none parks a live defect (§V.6).
...
- **Third stripper** — `packages/oxlint-plugins/recommended/scripts/guard-no-behavior.mjs:25-31` runs both comment regexes before replacing string literals, so `//` inside a scanned string mangles the line. Same class, inverted harm (false negative in a gate), different language surface.
