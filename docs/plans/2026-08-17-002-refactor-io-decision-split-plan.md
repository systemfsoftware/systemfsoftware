...
- R10. No blocking filesystem call remains in the migrated module or in the tests that exercise it, with one exception: specifier resolution. Node exposes no async resolver of either kind, and no base-flexible ESM resolver — measured on v24.19.0, `import.meta.resolve` returns a string synchronously and silently ignores a second base argument, so it cannot resolve from the declaring config file's directory. Resolution is performed at the shell boundary and wrapped there. Measured at implementation time: the production read is already `await fs.promises.readFile`, so this requirement's remaining work is entirely in the tests, where the sync calls actually live.
...
- KTD2. The shell stays a plain loop and gets no constructor of its own. `constructor-rule-boundary.md` rules that sequencing is inexpressible at the type level, so a shell constructor would be a marker that constrains nothing. The pure side keeps its constructor. Measured at implementation time: this package uses Effect as a schema library and nothing else — all 30 Effect imports are `effect/Schema`, `effect` or `effect/Exit`, there is no `Effect.gen` orchestration anywhere in it, and `config-reader.ts` is a typed-inject class with async methods. `@effect/platform` and `@effect/platform-node` are not dependencies. So the shell is a plain `async` loop, not an `Effect.gen` one: making it Effect-native would mean two new dependencies and a runtime boundary inside stryker's DI, which the Goal Capsule's stop conditions forbid and which buys the split nothing. Governs R9.
...
- Patterns to follow: read a neighbouring module in this same package that already consumes a decision and performs acts — the shape to match is the package's own plain-async idiom, not an Effect-native executor. `omp/plugins/omp-claude-compat/src/internal/run-user-prompt-submit-hooks.executor.ts` is the shape in Effect terms if it helps to see the pattern stated once, but do not import Effect to imitate it.
- Test scenarios:
