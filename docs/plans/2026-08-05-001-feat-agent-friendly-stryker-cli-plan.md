...
- Enrolling the new pure decisions in the mutation gate. `packages/stryker-js/core/stryker.config.json` still mutates only `src/reporters/test-contribution.ts`, so `resolveMode`, `isColorEnabled`, `isActionableStatus` and the R20 filter — all pure, all added by this work — sit outside it, and CONSTITUTION III.4 is unmet for them. `scripts/guard-mutate-scope.mjs` permits the widening (neither `output-mode.ts` nor `reporters/verdict-envelope.ts` carries a forbidden shell-cell suffix), so this is scheduling, not a rule conflict. Measured 2026-08-07 and deferred on the author's instruction: the run costs more wall-clock than the session had.
- Closing the startup silence. `configureStream` first runs about 15 seconds into a real run — module loading blocks the event loop for that long before any code of ours executes (measured 2026-08-07: a loop-lag probe recorded a single 15002 ms block, then nothing above 750 ms for the rest of the run). The stream contract is therefore honoured from stream start, but an agent invoking the binary still sees total silence for those 15 seconds, which is the window R17 and R18 exist to remove. Emitting the header before the dynamic import would need a lighter entry path in `bin/stryker.js`, and must keep the run id single-sourced.
