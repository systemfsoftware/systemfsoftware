...
- R2. The rule fires on hand-rolled primitives that have mature stdlib replacements: encoding (`btoa`/`atob`, `Buffer.from(..., 'base64')`/`toString('base64')`, hex loops → `jsr:@std/encoding`), dates (`new Date()`, `Date.now()`, `toISOString()`, `getTimezoneOffset` → `jsr:@std/datetime`/`Temporal`), ids (`randomUUID`/`uuid` → `jsr:@std/uuid`/`crypto.randomUUID()`), and crypto (`createHash`, `crypto.subtle` → `jsr:@std/crypto`) — all importable portably as `jsr:@std/*` (Deno) or `jsr:` catalog / `npm:@jsr/std__*` (Node/pnpm). None have Effect-native overlap.
...
- R4. The rule also covers `bytes` (manual `Uint8Array` concat/copy/equality loops → `@std/bytes`) and `path` (string concatenation with `"/"`/`"\\"` or `split("/")` path surgery → `@std/path`). Coverage beyond R2/R3 must not inflate the false-positive budget past the target.
...
- R8. The rule is falsifiable via manual regex checks and `omp ttsr test`/`scan` in any checkout — known-bad hand-rolled samples must trigger, known-good `@std/*` and Effect-native samples must stay silent — per `CONST-E1`/`CONST-E3` gate discipline. No checked-in fixtures are required; the plugin stays dependency-free and `turbo boundaries` stays green without adding `effect`.
...
- In scope: one portable markdown rule file plus its body guidance table (primitive → Effect / `@std/*` import → one-line example), verified via manual regex and `omp ttsr test`/`scan` in any checkout — no checked-in fixtures, no `effect` dependency.
- Out of scope: migrating existing hand-rolled call sites — the rule is forward-looking in any adopter. Also out of scope: requiring the consumer to have a specific `jsr:` catalog pin or Deno lock pre-existing, creating an oxlint rule parallel to the TTSR rule (TTSR is the chosen instrument per `shipped-runtime-enforcement`), and committing test fixtures that would pull `effect` and break `turbo boundaries`.
- Deferred to follow-up work: autofix/code-mod scripts, expanding coverage to `@std/fs`, `@std/streams`, `@std/yaml`, and editor-layer diagnostics — each needs its own false-positive measurement in a generic consumer setup.
