...
Seven review passes produced no evidence. A working emitter produced four results in under an hour, two of which refute earlier drafts of this brief. The workflow emitter now ships as repo tooling at `scripts/tools/workflow-emit.ts` with a `guard-script-provenance` entry, its gate at `scripts/guards/guard-workflow-authorship.ts`, and each declaration beside the cell it emits - the declaration is the source, the cell is the artifact. What remains beside this file in `docs/plans/2026-08-15-generated-authorship/` is the experiment that got there: `emit.ts` and `falsify.ts` for the executor probe, `workflow-falsify.ts`, the two executor `*.decl.json` probes, and `deno.json`.
...
**6. The authorship gate exists, and the 13 deletions landed behind it.** `scripts/guards/guard-workflow-authorship.ts` is the one presence check the brief permits: every `*.workflow.ts` under a package `src/` must have a sibling `*.workflow.decl.json`, and re-emitting it must reproduce the file byte for byte. It is wired through `gate:tasks`, so `pnpm check:local` and `check:ci` both carry it, and CI needed no workflow change — `reusable-checks.yml` already installs Deno and runs `pnpm check:ci` as the one definition. Observed red before and green after on both failure kinds: a one-identifier edit reports `hand-edited since emission`, an undeclared cell reports `hand-authored workflow cell` with the declaration path to write. That is what turns "unreachable" from a discipline into a property of the tree.
...
**Scope of the claim.** Two roles, four cells, 22 rules examined and 13 deleted. The remaining 87 rules and 10 roles are unexamined.
