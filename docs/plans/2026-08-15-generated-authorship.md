...
Seven review passes produced no evidence. A working emitter produced four results in under an hour, two of which refute earlier drafts of this brief. Artifacts sit beside this file in `docs/plans/2026-08-15-generated-authorship/`: `emit.ts` (emitter), `falsify.ts` (violation prober), `*.decl.json` (declarations), `deno.json`. They live here rather than under `scripts/`, which `guard-script-provenance` holds as a closed manifested set - an experiment is not repo tooling, and their production home when the work starts is `packages/effect-cell-gen`.
...
**6. The 13 deletions are withheld, and the reason is measured rather than cautious.** Those rules are unreachable _for emitted cells_. Nothing in this repository requires a workflow cell to be emitted: Task 2's authorship gate does not exist, so "unreachable" describes a discipline no gate enforces. Deleting on that basis is the exact failure this brief's own hunt list names - green by dropping a package from a gate's population. The blocker is concrete rather than open-ended: the gate must run in CI on the existing node/pnpm toolchain, the emitter is a Deno script, and `guard-script-provenance` holds `scripts/` as a closed manifested set - so the next unit is the emitter in a workspace package (CELL-A1 forbids `effect-cell-gen`, which already owns generated arbitraries), the three declarations beside the cells they emit, and a guard asserting every `*.workflow.decl.json` reproduces its sibling. Until that lands the 13 rules stay, doing no harm and no work.

**Scope of the claim.** Two roles, four cells, 22 rules. The remaining 99 rules and 10 roles are unexamined, and no rule has been deleted.
