...
- KTD11. Every `decision` and `error` schema is exported from a `*.schema.ts` module, so `@systemfsoftware/effect-schema-vite` generates the encode/decode round-trip laws for it with no hand-written test.
