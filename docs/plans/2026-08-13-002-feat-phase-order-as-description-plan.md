...
- **Dependencies.** U1.
- **Amended twice.** First: the original approach put the channel choice in `phase-outcome.kernel.ts` with property tests and a mutation verdict. Two findings killed it. `scripts/guards/guard-mutate-scope.mjs` lists `kernel` in `FORBIDDEN` and its header assigns mutation to `*.workflow.ts` only, naming a mutated kernel a wrong-observer error — so the stated verification was not merely unwired but prohibited; and there is no decision to extract, since the fold's position in the chain is static and no runtime input can make a `decode` Left travel forward, so a routing cell would manufacture a decision in order to have a mutation target. `KTD9`'s premise is withdrawn. Second: a layer's read now receives the previous layer's response. `U4` established that a later layer needs what an earlier layer decided, and with layers communicating only through observable side effects the sole alternative was to re-derive that decision in every following layer — duplicating it. Threading is licensed by the same canon atom the multi-layer shape rests on (`composite-operations.md` A6, Wlaschin): extra I/O mid-decision-making is allowed while the I/O segments stay separate from the decision segments. This unit takes no third amendment.
...
- **Approach.**
  ...
  3. Fold the layer list with `reduce`, which is sequential by construction and carries each layer's response into the next layer's read. The branches that remain are presence checks over optional slots, not decisions.
...
- **Test scenarios.**
  ...
  - Composition: a later layer receives the earlier layer's response, distinct from observing its write — proven by breaking the thread and watching only that scenario fail.
