...
- KTD2. **Base tree materialization.** The base hash comes from a throwaway `git worktree add --detach` of `<pinned-base-sha>` under a temp dir (amended post-review from the original `git archive` extraction — the worktree keeps materialization inside `git`'s own allow-run grant and was itself probed: probe A ran a dry run in a worktree copy and matched the checkout byte-for-byte, while probe K proved the same for a bare archive tree), followed by a dry run there; the head hash comes from the working tree. Keyed on the PR event's recorded `base.sha` — never on a moving ref resolved at job time, so a merge landing on main between event and check cannot infect the verdict. The temp worktree is removed after the verdict (a failed cleanup is logged, not fatal), and the base argument is resolved with `git rev-parse` first, failing closed with a clear message if absent.
- KTD3. **Turbo acquisition is the lockfile install, and nothing else.** The gate runs the binary the repo already pins: `pnpm install --frozen-lockfile` in the workflow, then `node_modules/.bin/turbo` by absolute path (the spawn, the lstat, the pin check, and the Deno `--allow-run` grant all name that one path), with `cwd` set into the base worktree for the base run. No `npx`, no registry fetch at verdict time — the executor is exactly the binary the workspace's own supply-chain gate installed. The selftest then verifies the running binary: it compares `node_modules/turbo/package.json#version` and the lockfile pin (asserting `lockfileVersion: 9.0` and the lockfile's importer path in the same stroke), and the verdict-time runs also check the engine's own dry-run self-report (`turboVersion`) against the pin, so a stale install, a pnpm upgrade changing the lockfile schema, or a silently re-resolved turbo all break the selftest rather than the verdict.
...
```mermaid
flowchart TB
  B[git worktree add base.sha] --> C1[turbo dry at base tree]
...
- `scripts/guards/check-changeset.ts` — rewrite (shebang `#!/usr/bin/env -S deno run --allow-read --allow-write=/tmp`; the workflow invokes it with `--allow-run=git,"$PWD/node_modules/.bin/turbo" --allow-read --allow-write=/tmp` — the exact shim path is the grant, nothing resolves through PATH; the selftest sub-mode requires `--allow-read` only).
...
2. Materialize base: resolve the base argument (`git rev-parse --verify` — a bare SHA from the workflow, or a ref for local runs), `git worktree add --detach` into a `Deno.makeTempDir` (a failed add removes the temp dir before rethrowing); wrap the whole verdict in `try/finally` that removes the worktree after the verdict (cleanup failure logged, not fatal).
3. Run `$PWD/node_modules/.bin/turbo run build --dry=json` twice — once with `cwd` at the base worktree, once at the working root (same absolute binary; the base tree needs no install). Parse `{tasks: [{taskId, package, directory, hash}]}`; validate every task carries a hash and that a run enumerating packages but parsing zero `#build` tasks fails closed as task-format drift; any failure throws and names the step (turbo exec vs output parse vs base ref). A run enumerating zero packages also fails closed — an empty enumeration is a broken premise, not an empty verdict.
...
1. The selftest parenthetical defining a git-spawn budget is dropped: the selftest spawns nothing (R10), and the main path's materialization makes exactly one git call (the worktree add; `diff` is pure git plumbing already counted by the workflow's `--allow-run`).
...
- The workflow's install step is `./.github/actions/install-deps` (frozen-lockfile); the selftest and main steps run `--allow-read` and `--allow-run=git,"$PWD/node_modules/.bin/turbo" --allow-read --allow-write=/tmp` respectively (no npx anywhere, no PATH-mediated turbo resolution); the main argument is the event base SHA.
