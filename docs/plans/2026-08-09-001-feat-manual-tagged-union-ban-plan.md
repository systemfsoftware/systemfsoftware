...
- **Product authority.** Repository owner. Grounded in `CONSTITUTION.md` I.5 (tagged union for mutually-exclusive states), II.5 (decode, never cast), REPO-S6 (enforcement ships in the published artifact), and the rulings `conventions-ruled-without-cell` and `enforcement-channel-ordering`.
- **Open blockers.** None.
...
- Rulings: `conventions-ruled-without-cell`, `cell-atlas`, `enforcement-channel-ordering`, `warn-severity-is-dominated`, `lint-rule-granularity`, `incremental-adoption-topology`, `gate-vs-suggestion-threshold`; `CONSTITUTION.md` I.5, II.5; root `AGENTS.md` REPO-S6.
