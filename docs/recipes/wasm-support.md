...
| `targetEnv`   | Based on `platform` | Controls the generated instantiation code. Defaults to `'browser'` or `'node'` matching the build's `platform`, and `'auto'` otherwise. `'auto'` detects the environment at runtime; `'auto-inline'` always inlines and decodes based on environment; `'browser'` omits Node.js builtins; `'node'` omits `fetch` (requires Node.js 20.16.0+). |
