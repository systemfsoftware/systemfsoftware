...
The best practices here relate to the [generated manifests](./manifests.mdx), which are in [preview](../releases/features.mdx#preview).
The API may change in future releases.
We welcome feedback and contributions to help improve this feature.

See [framework support](./mcp/overview.mdx#framework-support) for which frameworks generate manifests today.
...
In a `<script setup>` SFC, attach the docblock to a `defineOptions()` call.
