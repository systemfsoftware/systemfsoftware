...
- Test work is packed by measured duration. `scripts/tools/test-timings.ts plan` reads the newest `test-timings` artifact from `main` and packs packages into free `ubuntu-latest` jobs of at most 5 minutes. A package predicted over 5 minutes becomes vitest shards (`--shard=i/n --reporter=blob`), and a `merge` job per sharded package runs `vitest --merge-reports`, which judges conformance coverage and v8 coverage once over all shards. Every run writes the durations it measured, so later runs on `main` re-pack from them.
