...
# A dogfooding extractor package cannot bin-link itself: turbo self-cycle and the missing pnpm shim
...
**A self-hosting build gate resolves its engine by built artifact path, never by bin name.** The executable a build gate needs must already exist as a deterministic output of the same build (`tsdown` emits the CLI entry `main.mjs` before the dogfood step), so the gate invokes the artifact directly:

```json
"build": "tsdown -l warn && pnpm api:check",
"api:check": "node dist/main.mjs run --quiet",
"api:update": "node dist/main.mjs run --local"
```

No self edge exists in the graph, and the gate always exercises the code built seconds earlier — never a stale linked copy. Downstream consumers keep normal bin usage (`api-extractor run`); only the self-hosting package uses the path form. `api:check` runs in verification mode, so drift in the committed report fails `build`; `api:update` is the only path that rewrites it.
