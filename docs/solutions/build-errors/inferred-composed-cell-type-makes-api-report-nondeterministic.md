...
symptoms:
  ...
  - "the only diff is the member order inside the error-channel union of `const cell: Cell.Cell<...>`"
...
tags: [api-extractor, api-report, declaration-emit, tsdown, type-inference, union-order, effect-cell-types, determinism]
---
...
The `@systemfsoftware/api-extractor` package builds itself, then runs its own CLI over its own emitted declarations to regenerate its committed API report. The package's `build` script is `tsdown -l warn && node dist/cli.mjs run --local --quiet`. After the two Sandwich cells were composed into `Extractor.cell` as `announceRun.pipe(Cell.andThen(extractApi))`, the report changed on some builds but not others. It could never be committed in a stable state.
...
- Running `tsdown` once, then the CLI twice over the same `dist/`, produced identical reports. The analyzer is deterministic for a fixed input. The variation is in the emitted `.d.ts`, not in API Extractor.

## Solution

Declare the type of the exported composition `Extractor.cell` in the source. Declaration emit then copies the written annotation instead of printing the union the type checker inferred:
...
After the fix, eight consecutive builds produced one report hash.
...
- To check a report for determinism, run the package's real `build` script several times and compare hashes: `for i in 1 2 3 4 5 6 7 8; do pnpm --filter <pkg> build >/dev/null 2>&1; md5sum <pkg>/etc/<name>.api.md; done | sort | uniq -c`. Running the analyzer repeatedly over one fixed `dist/` does not reproduce the flake.
