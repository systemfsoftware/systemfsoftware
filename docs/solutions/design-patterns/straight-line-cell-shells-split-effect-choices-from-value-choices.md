...
4. **A zero-or-one loop is still a branch.** `Effect.forEach(Option.toArray(found), read)` runs the read only when `found` is present, which is an `if` in loop form. Gather the evidence itself as one effect instead. An optional file is one `readFileString` that recovers only the not-found reason: `Effect.asSome` plus `Effect.catchReason('PlatformError', 'NotFound', () => Effect.succeedNone)` (api-extractor's `readOptionalText`). Iterating over a list derived from evidence (every source a map names, every ancestor folder) is fixed work and stays in the phase.
5. **Never recover every error as "absent".** `Effect.orElseSucceed(() => Option.none())` around a read turns a permission failure into a missing file, which is a silent fallback. Recover the not-found reason only, and let every other `PlatformError` stay typed in the channel.
6. **Iteration whose next step depends on what was read is decisions.** The `extends` chain is a workflow deciding `FollowExtends | ChainComplete | CircularExtends | ChainMissing | ChainMalformed` from the evidence so far. Its cell's `FollowExtends` handler runs one step and re-enters the cell (`config-chain.cell.ts`).
7. **The plan builder is the decide.** When a write phase runs a plan, the pure function that builds the plan is the `decide` of the cell that runs it (`write-plan.workflow.ts`'s `buildWritePlan`, run by `write-plan.cell.ts` with one exhaustive handler per step kind). A workflow whose decision just echoes its command is a tautology. Its property can only restate the implementation, so delete it.

A plan built as data also gives you one place to apply a cross-cutting filter. api-extractor applies verbosity once, to the finished step list in `buildWritePlan`. With per-site filtering, every new emit site has to remember the filter.
