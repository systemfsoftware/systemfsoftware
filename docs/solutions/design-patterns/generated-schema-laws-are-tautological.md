...
- **Stale rationale to repair:** `packages/oxlint-plugins/test-placement/AGENTS.md` rule TP4 justifies the `*.schema.test.ts` ban by claiming the generated pair "already covers every exported schema." This learning shows that is false in the rejection dimension. The ban remains correct policy; the stated reason needs replacing with the tautology, and the rule should acknowledge that `.schema` is now a property-carrying cell.
