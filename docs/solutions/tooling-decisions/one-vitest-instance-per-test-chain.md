...
1. pnpm keys a package build by name + version + resolved peer context. The workspace's `@effect/vitest` pulls vitest with one peer closure; the workspace root's own vitest dev-dependency carries a different peer closure (`@vitest/coverage-v8` vs a browser-playwright variant). Two physical installs result: same version `4.1.10`, different `.pnpm` suffix hash.
...
Repair is `pnpm dedupe`: one physical vitest, one module registry, state keyed once.

## Architectural invariants

- **Single-instance invariant:** for every workspace importer that resolves `@effect/vitest` (directly or through workspace links that themselves declare it), the vitest copy `@effect/vitest` loads and the vitest copy the importer runs must realpath to the same file. Equality is per-importer and per-physical-path; version-string equality proves nothing.
