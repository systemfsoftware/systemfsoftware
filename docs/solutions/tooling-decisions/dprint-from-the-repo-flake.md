...
- **dprint exists only as a flake package.** `flake.nix` exposes `.#dprint` and a dev shell (dprint + comment-checker + node 24 + deno; pnpm is deliberately absent — `packageManager` pins pnpm and corepack resolves it). `bin/dprint` is the single entry point: it takes the first dprint on PATH (dev shell, direnv, CI's `nix build --print-out-paths` store path) and otherwise `nix run`s the flake; with neither, it prints the error message pointing at CONTRIBUTING.md. Never add a `dprint` npm dependency back — the `allowBuilds` entry is gone with it.
- **The second native tool follows the same shape, one level up.** `comment-checker` is a fixed-output fetch of its release asset, its per-target digests read from the pinned input's `nix/release-hashes.json` (so `nix flake update comment-checker` moves version and digests together), and it ships as the sandboxed wrapper — `.#comment-checker` — while `.#comment-checker-unwrapped` stays exposed for probing. The sandbox's path guard is recorded in [`../security-issues/unescaped-cwd-in-a-seatbelt-profile.md`](../security-issues/unescaped-cwd-in-a-seatbelt-profile.md).
