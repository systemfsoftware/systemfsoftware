...
1. **A byte-identity pin is a conjunction over the pairs it enumerates.** The drift guarantee is `identical(A,B) ∧ identical(A,C) ∧ …` for exactly the copies the pin names. Any copy outside the enumeration has a vacuous guarantee: nothing is false, nothing is checked.
2. **A mirror whose justification is a build constraint inherits every build decision.** The mirror exists so published dists do not carry a dependency on another plugin. That goal never requires duplicated _source_ — only duplicated _artifacts_.
