...
3. So the rule never runs on the violation it exists to catch: an author whose module holds an
   Effect runtime import simply does not write `.kernel.ts`. The rule has full precision against
   nothing.
...
suffix boundary is invisible when crossed; the manifest boundary is a type error.
...
Aiming wide does not merely find more: mutating a module that performs I/O yields a mutant that
hangs rather than one that dies, so the timeouts mark the effectful boundary empirically instead of
by label, and the run reports where aiming stops being meaningful.
...
- The purity rules made uniform. No effect-runtime import, no ambient impurity and no `throw` are
  things to want of every module; routing them by suffix left every other module free to do the
  thing.
...
The labels and the rule surface routed on them collapse to the type system, the module graph, and two
manifest facts.
...
module. Two found none across the surface they audited. The third produced three, argued in full,
and all three re-keyed:

- **The rule against constructing a driver here** — that is the FCIS import direction. Keyed on the
  manifest, a driver import is `TS2307` rather than a lint report.
- **The rule limiting a port to one Layer export** — the unit is the **port**, not the file. Keyed
  per-port over the graph, a composition root binding many different ports passes, while one file
  exporting three Layers for a single port is caught. A suffix rule permits that file.
- **The rule requiring a match to be exhaustive or else** — "handler versus workflow" was a proxy.
  The real criterion is whether the module **owns** the matched union: own it and `exhaustive` is
  safe, import it and a variant added elsewhere breaks this build remotely. Keyed that way it also
  catches the workflow exhaustively matching a foreign union, which a suffix rule cannot see.
...
The same failure recurs inside a rule written to enforce this document:
`behaviour-exercises-use-case` demanded that a `*.integration.test.ts` import a module whose basename
ended in `Executor`, `Handler`, `Adapter`, `Store` or `Middleware`, which is this document's defect
with the label moved from the dotted position into the module name. A role word in a shell module's
name is good documentation; treating it as evidence is the same mistake.

A probe settled it in one run: a pure module, holding one total function and no import at all,
reached by a behaviour test that touched nothing else. The rule stayed **silent**. The gate certified
"this test drives a use case through the I/O sandwich" on the strength of five letters the author
typed, and no step recomputed whether the module did any I/O — a check keyed on a value its own
author supplied, in a rule written to enforce this document.
