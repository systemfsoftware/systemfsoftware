...
1. **Coverage can be extrinsic to the symbol it covers.** Coverage of `ruleOfSchemas` lived in another module's in-source block, as a side effect of a fixture that called it once; no in-source block ever lived beside `ruleOfSchemas` itself. Coverage filed in a _different_ module than the symbol is invisible to any per-module reasoning about a move.

2. **The boundary fix deleted the last caller.** An extraction's new package must not depend on the origin, so the one cross-package call is deleted rather than carried across the new edge. Deleting it is right, and it is also the moment total coverage of the residue reaches zero. The two facts get decided in different steps, and nothing joins them.
