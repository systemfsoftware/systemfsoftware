...
| # | Form of type claim                                                                                                                                                                                                                                                                       | Live instance in this workspace (re-searched 2026-09-25)                                                                                                                                         | Refusing channel                                                                                                                                                                                                                                       |
| - | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1 | The unchecked cast: an `as` (or `as unknown as`) asserts a shape nothing verified                                                                                                                                                                                                        | None in production source. The instances first recorded here lived in the Stryker fork, which left this repository                                                                               | `typescript/no-unsafe-type-assertion` refuses a single cast except under a suppression comment; `ban-unknown` (`@systemfsoftware/oxlint-plugin-cell-architecture`, `error` in the recommended preset) refuses the `as unknown` step of the double cast |
| 2 | The check-skipping constructor option: the generated default constructors accept a documented option that trusts the input and skips member checks, and the vendor documentation recommends it whenever the author trusts the data — a judgement no type can make on the author's behalf | `readStep` in `@systemfsoftware/effect-daemon-spec` constructs `SupervisionStep` with `{ disableChecks: true }`                                                                                  | None                                                                                                                                                                                                                                                   |
| 3 | The declared type with an inadequate predicate: `Schema.declare` publishes a type whose predicate admits far more than the type says                                                                                                                                                     | None in production source                                                                                                                                                                        | None                                                                                                                                                                                                                                                   |
| 4 | Unknown admitted at a boundary: a record or rest member typed `Unknown` passes any payload into a position the surrounding type treats as shaped                                                                                                                                         | `DehydratedAtom`'s `value` member in `@systemfsoftware/effect-atom`, and the optional `cause` member carried by tagged errors across the workspace                                               | None — the admission is deliberate; a dehydrated value and a wrapped cause are unknown-shaped by design                                                                                                                                                |
| 5 | The zero-filter brand: `S.brand` applies a nominal tag with no runtime filter, so a branded type claims a distinction nothing checks                                                                                                                                                     | None as shipped production code: every brand in `@systemfsoftware/hex-schema` follows a check. The form is one unfiltered `S.brand('Name')` away (the ecosystem's own examples are exactly that) | None                                                                                                                                                                                                                                                   |

## What a gate could do with them

A text gate can refuse forms 1 and 2 outright: a cast is a syntactic event, and the skipping option is a named argument. Form 5 is nearly so — a brand with no filter in its pipe is detectable. Forms 3 and 4 are legal TypeScript and deliberate modelling; a gate can only require justification for them, because the form is not the defect — the predicate's adequacy is, and no textual rule can measure that.
...
Unowned, all but the double cast — and form 2 now has a shipped instance nothing refuses. This is the honest record, not a coverage claim: the surfaces searched were the guard directory, the root script chain, the lint rule inventory, and a workspace source search per form; absence of a firing outside those surfaces is not proven.
...
- `docs/solutions/architecture-patterns/make-boundary-owns-a-decision.md` — the precedent shape for recording a deleted instrument's uncovered obligation
