...
| 3 | The declared type with an inadequate predicate: `Schema.declare` publishes a type whose predicate admits far more than the type says                                                                                                                                                     | `Runner.schema.ts` in `packages/stryker-js/stryker-js-vitest-runner` — declares "the project-local vitest/node module" with a predicate that admits any object                                                      | None                                                                                                                                                                       |
| 4 | Unknown admitted at a boundary: a record or rest member typed `Unknown` passes any payload into a position the surrounding type treats as shaped                                                                                                                                         | `Config.schema.ts` (`ConfigDocumentSchema`) in `packages/stryker-js/stryker-js-engine`, and every `StructWithRest` rest member (for example `openStruct` in `Schema.schema.ts` of `packages/stryker-js/stryker-js`) | None — the admission is deliberate; plugin keys and config documents are unknown-shaped by design                                                                          |
