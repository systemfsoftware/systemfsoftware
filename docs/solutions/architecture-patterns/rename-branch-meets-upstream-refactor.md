...
A branch that mass-renames files (kebab decision stems locked to exports) collides with main when main independently refactors the same files. `git rebase` replays every cutover commit against shifted ground and produces ~35 conflicts per commit; `git merge` collapses them to one round but marks each collision `UU`/`DU`/`UD` with both sides half-right: the branch owns the naming, main owns the newer content.
