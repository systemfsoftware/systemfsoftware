...
The constitution (`CONSTITUTION.md`, whose `gate:` value on each rule block is the whole source) designates `gate: review` for 22 of its 38 rules — "enforced by review." The remaining 16 are mechanized: 12 lint, 2 type-checker, 2 mutation. The mechanized rules fail builds; the review-gated rules are judgment calls no command can make. A lint rule catches `as any` (CONST-B5). It cannot catch "this change patches a symptom instead of fixing the root cause" (CONST-S1) or "this test buries logic in I/O" (CONST-T15).

`gate: review` means "a human or reviewer checks this" — and nothing in this repo's agent loop reviews against it. The rules are dead text: stated, cited, never enforced.

## Guidance

A `WATCHDOG.md` at the repo root `@import`s `CONSTITUTION.md` into the omp advisor's system prompt. No such file exists — not at the repo root, not in the harness config directory — so the description below is a design that has never run. The advisor — a separate model with its own session and `read`/`grep`/`glob` tools — reviews every code delta against the 22 review-gated rules and raises `concern` (violation), `nit` (borderline), or stays silent (conforms). The 16 mechanized rules are excluded; the advisor does not re-check what the toolchain already catches.

### Key design decisions

1. **@import amortizes the constitution.** The full `CONSTITUTION.md` (~415 lines) is loaded once into the advisor's system prompt at session start, not re-read per turn. The advisor carries it in context for every review.

2. **The advisor is external, not circular.** It is a separate model with its own session — not the primary agent judging itself (CONST-E9). The primary agent could edit `WATCHDOG.md` or `CONSTITUTION.md` (both editable), but those changes go through review, and the advisor would see the change in the next transcript delta. Bounded circularity, not the unbounded kind.
...
5. **Model choice matters for judgment calls.** The 22 review-gated rules are the hard ones — the ones that couldn't be mechanized. A flash model with thinking off is the lowest-reasoning configuration. It catches obvious violations (handler with business logic → CONST-B1, symptom patch → CONST-S1) but misses subtle ones (I/O interleaved in a refactor → CONST-B3). Use a mid-tier model with thinking on for the advisor role.

## Applicability

Use this pattern when a doctrine document designates `gate: review` (or equivalent) for rules that are judgment calls. Do not use it when a rule can be mechanized — make it a lint/type-checker/mutation gate instead, per the mechanism ladder in `repos/constitution/ENFORCEMENT.md`. A rule that can fail a build should fail a build; the advisor is the fallback for rules that can't.

## Not yet verified

Nothing here was ever activated: no `WATCHDOG.md` exists in this repo or in the harness config directory, so the constitution has never reached an advisor's window through this route, and no advisor has reviewed a delta against these rules. Activation would take the file plus `advisor.enabled: true`, a model for `modelRoles.advisor`, and `task.agentAdvisor` entries for subagent advising. The pattern's effectiveness (false-positive rate, concern quality, subagent coverage) is unmeasured.
...
- [Rule admission severity and accretion](../tooling-decisions/rule-admission-severity-and-accretion.md) — "an honest advisory belongs in review"; this pattern is the review surface that doc called for, still unbuilt.
