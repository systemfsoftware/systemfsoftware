...
`@systemfsoftware/effect-cell-types` publishes a `Cell.layer` one-sandwich constructor, an internal assembler that chains the phases in one built-in order, a `Cell.canonical` value built through it, and `Cell.vocabulary`, a fold over that value's description carrying `module`, `ioCells`, `phases`, `byKind` and `composer`. Five axes come off the fold as data: the phase names, their purity, their order, the owning module name, and the I/O-cell classification.

Three packages outside the description derive their whole behaviour from that one value, and naming them beats counting them, because an enumeration decays quietly: a FastCheck generator that produces random-but-valid layer specs as the interpreter's property oracle, the `cell-vocabulary` lint plugin deciding which calls a pure phase body may make, and the `effect-daemon-spec` supervisor whose executor runs a Cell per supervision event.
