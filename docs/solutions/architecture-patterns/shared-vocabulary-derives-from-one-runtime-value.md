...
That is the part worth carrying away. Neither carrier is free, and the second one's cost is not visible in the diff that introduces it — it shows up later, in a package nobody has written yet. So a consumer-side carrier is only honest once something fails when the delivery is missing: here, the lint-coverage guard — a script since deleted — grew a positive check that a package owning a consumer-delivered cell also loads that cell's plugin, observed failing when one consumer's entry is removed. Trading a drift gate for a delivery gate is a fair trade; trading a drift gate for a convention is not.
