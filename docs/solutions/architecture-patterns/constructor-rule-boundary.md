...
The repo enforces a thirteen-suffix cell taxonomy — `acl, adapter, executor, handler, kernel, middleware, observer, policy, schema, shape, state, store, workflow` — through depth-0 lint rules carried by the `@systemfsoftware/oxlint-plugin-*` packages. A cell's contract is a pile of rules, each one a proxy for a property the suffix names. The question this work answered was whether type-level constructors could replace the rules, one per cell: give each suffix a `make` whose parameter type rejects what the rules reject, and retire the rules it subsumes.
...
A constructor earns existence only by computing something the author cannot write. `Workflow.make` earns it; a `schema` constructor cannot, because every rejection it would produce comes from its parameter type alone, which an author-written `S.Schema` annotation rejects identically — it computes nothing. A brand on the return would record only that the constructor had been called: provenance rather than a proposition.
...
Conclusion, measured against a **hand-authored** file: **zero rules retired, one rule required.** The `.tst.ts` assertions stand as what the constructor promises a consumer, not as replacements. The obligation that rule carried — forcing the constructor, so that the annotation form, a non-`make` initializer, and a local `type Workflow<...>` copy of the contract are refused, because only the constructor's inference is the force — now sits in three rules, all `error` in `configs.recommended` of `@systemfsoftware/oxlint-plugin-dmmf-workflow`: `workflow-file-make-presence` (a `<stem>.workflow.ts` file constructs its decision with `Workflow.make`), `workflow-file-export-topology` (exactly one non-schema value export, re-exports forbidden), and `damp-workflow-stem` (the file's stem is the kebab phrase its single value export camelCases to).

**Both observers share a premise, and it no longer holds for this cell.** Every input in both experiments is something an author _writes_: a total decision written at the definition site, a private `async` helper, a `Promise` annotation in a non-decider position, an annotation form in place of `make`. Above both observers sits the compiler channel. `Workflow.make` refuses an uninhabited or untagged channel at the construction site through marker members whose property NAME is the diagnostic — `__WORKFLOW_ERROR_CHANNEL_IS_NEVER__`, `__WORKFLOW_DECISION_CHANNEL_IS_NEVER__`, `__WORKFLOW_ERROR_CHANNEL_CARRIES_NO_TAG__` — so a total decision or a `Promise`-returning decider is a compile error where it is written, not at a call site that never comes. The shared effect preset carries the impurity half: the `diagnosticSeverity` map in `@systemfsoftware/tsconfig/effect` — the `@effect/language-service` plugin configuration a package extends — sets 79 of 86 rules to `error` (counted when this was written), and `asyncFunction`, `globalFetchInEffect`, `globalDateInEffect`, `globalRandomInEffect` and `processEnvInEffect` are all `error`, so a workflow that declares `async`, awaits, or reads the clock, the environment, or randomness is rejected by the compiler across every package. What the compiler channel does not reach is a decider that throws; that obligation has no owner. The experiments are not defeated — they remain the correct account of what a type and a suffix rule each reach on a file a human types. What changed is that the compiler channel now reads the file where it is written.

**(b) Why sequencing is not constructible.** The I/O sandwich — read → decode → decide → shape → write — is an _ordering_ constraint, and an indexed `Cell<Phase, A, E, R>` threaded through `Effect.gen` (in the `effect` package) cannot express order. `Effect.gen` unions every yielded effect into one `Eff` type parameter and extracts `E`/`R` from it, so type information crosses `yield*` fine — that is not the obstacle. The obstacle is that `Eff` is a **union**, and union is commutative and idempotent: a phase index threaded through `gen` can express which phases are _present_ but never the _order_ they occurred in. `read → write` and `write → read` are the same union, so the sandwich order is unreachable through the generator. An index licences `write → decide` explicitly and therefore forbids nothing. That is why indexing is rejected on derivation — not on `yield*` erasure — and why a sequencer cannot be constructed: the executor and handler cells _are_ shells.
...
2. **Does Effect already supply the composer?** If yes (`S.Schema` for schema, `S.decodeTo` for acl, `Layer.effect` for adapter), the constructor wraps what exists and computes nothing. Stop.
...
Counter-indications that make a constructor the wrong instrument, even when a cell earns one: claims that need the whole file (a `Promise` reference in a helper, a local type alias, an `await` outside the decider); file-level export cardinality; import-graph and module-scope-state facts; and any ordering constraint, which no type over a value can hold. Where the doctrine is real and not type-expressible, the question is which channel can read it. On a hand-authored cell, keep the rule and fix its message rather than its instrument. Where the compiler already decides — a `Workflow.make` channel marker or an `error` entry in the language-service `diagnosticSeverity` map — a rule over that class can never fire, and the compiler is the stronger channel and the reason a walker is the last resort rather than the default. Where no channel reaches the constraint — a workflow decider that may throw — the obligation is unowned, and that is where a depth-0 rule is the walker that must still read it.

## Examples

**Before** — the annotation form. The type binds only here; a `never` channel never reaches the `Uninhabited*` markers, so a total decision and a `Promise` return slip through at the definition site:
...
**After** — the constructor form, the command's schema class at position 0. `D` and `E` infer from the decider's return, the `never`-markers derive, and the file reads as the ecosystem it publishes into:
...
The constructor's rejections were pinned by `@systemfsoftware/effect-cell-types`'s `Workflow.tst.ts`, quoted below as they read when this was written. That file has since been folded into `workflows-surface.tst.ts`, which still refuses the `Promise`-returning decider (`decidePromiseOverTagged`):
...
- PR #135 — shipped `Workflow.make` and `Policy`; both experiments in **Why This Matters** are reproducible from that tree.
- Issue #138 — the `kernel` cell's observer gap: the same "which observer reads this cell" question.
