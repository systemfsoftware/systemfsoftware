...
Counter-indications that make a constructor the wrong instrument, even when a cell earns one: claims that need the whole file (a `Promise` reference in a helper, a local type alias, an `await` outside the decider); file-level export cardinality; import-graph and module-scope-state facts; and any ordering constraint, which no type over a value can hold. Where the doctrine is real and not type-expressible, the question is which channel can read it. On a hand-authored cell, keep the rule and fix its message rather than its instrument. Where the compiler already decides — a `Workflow.make` channel marker or an `error` entry in the language-service `diagnosticSeverity` map — a rule over that class can never fire, and the compiler is the stronger channel and the reason a walker is the last resort rather than the default. Where no channel reaches the constraint — a workflow decider that may throw, or that reads ambient impurity — the obligation is unowned and is named as such in `docs/2026-08-15-orphaned-cell-constraints.md`, and that is where a depth-0 rule is the walker that must still read it.
