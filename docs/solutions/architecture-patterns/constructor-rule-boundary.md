...
**Both observers share a premise, and it no longer holds for this cell.** Every input in both experiments is something an author _writes_: a total decision written at the definition site, a private `async` helper, a `Promise` annotation in a non-decider position, an annotation form in place of `make`. A `*.workflow.ts` is now emitted from a declaration and `guard-workflow-authorship` requires it, so a third observer sits above both — the declaration language, which has no field for any of them. `workflow-declaration-form` cannot be violated because the emitter always writes `Workflow.make`; `workflow-no-async` cannot, because a declaration cannot name `async`, `await` or `Promise`; `workflow-either-inhabited` and `workflow-schema-required` cannot, because the channels and their variants are what the declaration _is_. Those four retired with nine others. The experiments are not defeated — they remain the correct account of what a type and a suffix rule each reach on a file a human types. What changed is that nobody types this one.
...
Counter-indications that make a constructor the wrong instrument, even when a cell earns one: claims that need the whole file (a `Promise` reference in a helper, a local type alias, an `await` outside the decider); file-level export cardinality; import-graph and module-scope-state facts; and any ordering constraint, which no type over a value can hold. Where the doctrine is real and not type-expressible, the question is who authors the file. On a hand-authored cell, keep the rule and fix its message rather than its instrument. On an emitted one, the declaration decides it before the file exists and a rule over that class can never fire — which is a stronger channel than either observer here, and the reason a walker is the last resort rather than the default.
