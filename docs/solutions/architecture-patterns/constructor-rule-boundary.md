...
The repo enforces a thirteen-suffix cell taxonomy — `acl, adapter, executor, handler, kernel, middleware, observer, policy, schema, shape, state, store, workflow` — through depth-0 lint rules carried by the plugin packages under `packages/oxlint-plugin`: the roughly 111 rules across 20 plugins this work counted is a snapshot taken before the cell-role suffix fleet was deleted, and nothing in the tree re-derives it. A cell's contract is a pile of rules, each one a proxy for a property the suffix names. The question this work answered was whether type-level constructors could replace the rules, one per cell: give each suffix a `make` whose parameter type rejects what the rules reject, and retire the rules it subsumes.
...
Counter-indications that make a constructor the wrong instrument, even when a cell earns one: claims that need the whole file (a `Promise` reference in a helper, a local type alias, an `await` outside the decider); file-level export cardinality; import-graph and module-scope-state facts; and any ordering constraint, which no type over a value can hold. Where the doctrine is real and not type-expressible, the question is which channel can read it. On a hand-authored cell, keep the rule and fix its message rather than its instrument. Where the compiler already decides — a `Workflow.make` channel marker or an `error` entry in the language-service `diagnosticSeverity` map — a rule over that class can never fire, and the compiler is the stronger channel and the reason a walker is the last resort rather than the default. Where no channel reaches the constraint — a workflow decider that may throw, or that reads ambient impurity — the obligation is unowned — it was recorded in `docs/2026-08-15-orphaned-cell-constraints.md`, a note no longer in the tree — and that is where a depth-0 rule is the walker that must still read it.
...
- PR #135 — shipped `Workflow.make`, `Policy`, and the since-deleted `workflow-declaration-form`; both experiments in **Why This Matters** are reproducible from that tree.
