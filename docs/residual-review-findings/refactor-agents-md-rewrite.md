...
**100/100 as finally measured, from 80/100 with bottleneck `hierarchy` on the first pass.** Both causes are recorded below; findings were adjudicated, not obeyed.
...
The first cause was mine and the validator was right: porting the guards to Deno left the rewritten root citing `.claude/hooks/guard-protected-writes.sh` in four gate fields for files that no longer exist. Repaired by naming the `.ts` paths. This is the check earning its place — a gate field pointing at a deleted file is exactly the failure `ADOC-A1` describes, and nothing else in the chain caught it.

The second was a measurement artifact, and is recorded because it would otherwise read as a repair. `packages/daemon/effect-daemon-spec/AGENTS.md` names `dist/index.d.ts` and `dist/effect-daemon-spec.d.ts`, which do not exist in a clean tree and do exist after a build. The first run measured an unbuilt tree and reported them stale; the final run followed `pnpm check`, which builds, and the FAIL cleared. Nobody fixed that leaf — it is under `packages/`, denylisted here because a concurrent agent is live in it (`REPO-M1`). The standing caveat is the validator's, not the leaf's: its stale-path check is build-state dependent and will flap on a fresh clone.
...
Every guard produced a real exit code against the real environment in this session (`USER-V4`) via `deno task test` in `.claude` — **30 cells, all green**: each guard fires on a known-bad payload and stays silent on a known-good one, with a control per guard proving it is not a blanket blocker, and three cells spawn the hooks as executables to assert the exit-code contract itself (`2` refuses, `0` allows, an unparseable payload does not brick the write path).

The guards ship as Deno TypeScript, not bash. Each is an I/O sandwich — an exported pure decision the suite calls directly, and a thin shell that reads stdin and exits — so the fixtures test the decision rather than a subprocess transcript. Primitives come from `@std`: `@std/path/posix` `relative`/`isAbsolute`, `@std/fs` `existsSync`/`ensureDirSync`, `@std/streams` `toText`, `@std/testing/bdd`. `jq` is no longer a runtime dependency of any hook. `.claude/deno.jsonc` pins them with a committed `deno.lock` and declares no package identity, so `deno publish` refuses and the tree cannot reach JSR.

The port fixed a real defect in the bash original beyond typing: path scoping was string slicing on the project prefix, so `/repo/../etc/hosts` resolved as the in-repo path `etc/hosts`. `relative()` returns a `..`-prefixed path for an escape, which is now rejected, and a cell asserts it.

Four cells are commands this repository has actually run, taken verbatim from the session transcript rather than composed by the guard's own author — the `AB8` correction. That is not decoration: it caught a live false positive. `qmd query --help` carries no `-c`, was run earlier in this session, and the first version of the qmd guard refused it. Fixtures written in the author's own words had all passed. A second defect died with the bash implementation and is recorded because the class outlives it: `set -o pipefail` with a non-matching `grep` made that guard return 1 on **every** Bash call containing no `qmd`.

The suite was falsified rather than trusted (`USER-V5`). Eight mutants were seeded one at a time across the three guards — dead `REPO-S3` branch, `REPO-W4` ignoring the stamp, traversal escape allowed, `Edit` and `MultiEdit` shapes ignored, `--help` exemption removed, pipeline cut widened, stamp accepting any collection — and **all eight were killed**.

**Dispatch limitation, recorded rather than glossed.** The fixtures prove each guard's logic against real payload shapes, and the executable cells prove the exit-code contract. They do not prove the harness invokes them, because this session's project directory is the primary checkout, not the worktree. Dispatch for the `Write|Edit|MultiEdit` matcher is evidenced by its sibling `deliver-leaf-agents-md.sh` firing live in this session from the identical `type: command` entry. Dispatch for the new `Bash` matcher has no such precedent in this `settings.json` and stays unproven until a session runs rooted at the worktree.

`deliver-leaf-agents-md.sh` is the one hook still in bash. It predates this change, is proven live, and has no fixtures of mine; porting it blind would trade a working gate for an unfalsified one.
