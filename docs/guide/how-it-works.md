...
| `type: "module"` in package.json                                             | Use `.js` extension for ESM output (instead of `.mjs`) when `fixedExtension` is disabled        |
| No `entry` specified, but `src/index.ts` exists                              | Use it as the default entry point                                                               |
| `platform: "node"` (the default)                                             | Enable [`fixedExtension`](../reference/api/Interface.UserConfig#fixedextension) (`.mjs`/`.cjs`) |
| `exports: true`                                                              | Generate the `exports` field in package.json                                                    |
...
| [`deps.onlyImport`](../options/dependencies.md#deps-onlyimport)               | Whitelist of packages the emitted output is allowed to import at runtime. Any other package import in the output causes an error.                                               |
...
- tsdown analyzes your entry points and output files, then writes the `exports` field in your `package.json` automatically.
- Top-level `main`, `module`, and `types` fields are generated automatically unless your build is ESM-only. Use `exports.legacy` to control this explicitly.
...
- **Target** — Inferred from your `package.json` `engines.node` field. If neither `target` nor `engines.node` is set, no syntax downleveling is applied. See [Target](../options/target.md).
