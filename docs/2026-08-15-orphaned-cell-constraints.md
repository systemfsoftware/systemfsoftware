...
| O8  | a workflow file exports exactly one workflow function                                                 | `architect-workflow.md`   | `workflow-single-function-export` was keyed on the `.workflow.ts` filename and no current rule reads the export arity of a workflow file                                                                                                                             |
| O9  | a workflow's single parameter is a tagged command class                                               | `architect-workflow.md`   | `workflow-command-object` was keyed on the `.workflow.ts` filename and no current rule checks the command parameter's shape                                                                                                                                          |
| O10 | a workflow is produced by `Workflow.make`, not an inline function, a local type, or a foreign `.make` | `architect-workflow.md`   | `workflow-declaration-form` was keyed on the `.workflow.ts` filename and no current rule checks how a workflow is declared                                                                                                                                           |
| O11 | a workflow body is single-path: no if/switch/ternary or loop, dispatch is exhaustive `Match`          | `architect-workflow.md`   | `workflow-single-path` was keyed on the `.workflow.ts` filename; `workflow-match-exhaustive` checks the `Match.value(...).pipe(...)` dispatch form but not the presence of branching control flow                                                                    |
...
O3, O4, O5, O7 and the export-authoring-form set O8-O11 are interior. Each is decidable at depth
zero — a `throw` statement, a call to `next`, a `try` block, a module-level initializer, a second
exported function, a bare parameter annotation, a value that is not a `Workflow.make(...)` call,
an `if`/`switch`/ternary/loop in the body — and undecidable one indirection out, behind a wrapper
call. A rule for one of these is worth having at exactly the depth it can
