...
One Prisma schema reached through two roots is one file. It is parsed once and both populations are served by that one result, so a package installed under `node_modules` and rooted again at its workspace source does not report the models it declares as declared twice. The location printed for such a file is one of its spellings rather than each population's own, and it opens the same file either way.
