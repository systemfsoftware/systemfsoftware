...
3. `.decide(workflow)`: A pure decision `Workflow` created with `Workflow.make`. Decision refusals (`Result.fail(DomainError)`) are domain _outcomes_, not execution defects: they pass to `encode` and `write` to record domain events or audit logs before responding.
