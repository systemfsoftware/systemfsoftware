...
Requirements are the handoff: whether a human or an AI wrote them, a human must review them last. That reviewed layer is the source of truth, and the agent builds everything below it with the graph counting what actually arrived.

- Hand over the requirements, and the agent writes the rest.
- Hand over raw idea notes, and it writes the requirements too. The document chains below are what keep that second form safe.

Keeping the versioned specification as the single source of truth is Spec-Driven Development, the practice GitHub Spec Kit named and AWS built Kiro around. SDD uses the specification as an input and does not count whether it was implemented. That is the gap this fills, which is why adopting it costs a configuration block rather than a workflow: nothing here requires a new process.
...
The arm with the graph finished on between a thirteenth and a fifth of the tokens on every subject, and the largest saved the most: the ERP closed on 411M tokens and $4.96 against 5,449M and $68.72. Four subjects and one engine do not make that a law, so treat the saving as measured on these four rather than as one you can assume.
...
- [Examples](/docs/evidence/examples) is the finished shape this ladder climbs toward, on a backend, a frontend, and a novel.
