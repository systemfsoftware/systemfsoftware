...
//
// stripResolveTtsxLauncher can be right while the command still spawns a bare
// `ttsx`, because the launcher is chosen inside stripTtsxCommandContext rather
// than passed to it. This pins the wiring: a resolved `.js` launcher also has
// to be handed to node, which stripShouldRunThroughNode decides from the
// extension.
//
