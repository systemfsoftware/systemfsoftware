...
| `targetEnv`   | 根据 `platform` 推断 | 控制生成的实例化代码。默认与构建的 `platform` 对应，为 `'browser'` 或 `'node'`，其他情况为 `'auto'`。`'auto'` 在运行时检测环境；`'auto-inline'` 始终内联并根据环境解码；`'browser'` 省略 Node.js 内置模块；`'node'` 省略 `fetch`（需要 Node.js 20.16.0+）。 |
