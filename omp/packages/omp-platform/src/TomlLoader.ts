...
/**
 * Port: layered project config. The live adapter reads the TOML layers.
 */
export class ProjectConfig extends Context.Service<
  ProjectConfig,
  {
...
const makeTomlLoader = (home: string) =>
  Effect.gen(function*() {
    ...
    return ProjectConfig.of({
      load: Effect.fn('ProjectConfig.load')(function*(cwd: string) {
...
/**
 * Live `ProjectConfig` layer anchored at `OMP_USER_CONFIG_HOME` when set, else
 * `os.homedir()`, resolved at layer-build time.
 */
export const ProjectConfigLive: Layer.Layer<ProjectConfig, never, FileSystem.FileSystem | PathModule.Path> = Layer
  .effect(
    ProjectConfig,
