/**
 * Feature tests for the layered TOML config (U5).
 *
 * Covers the three-layer chain (user → project → local) with per-key
 * override, per-layer fail-open, and the per-cwd cache. The pure
 * precedence contract is pinned by direct kernel tests in the first
 * `Feature` block; the layered-integration scenarios live in the
 * second.
 */
...
import { mergeByOverride } from '../src/toml-loader-merge.kernel.js'
import { makeTomlLoaderLive, TomlLoader } from '../src/toml-loader.adapter.js'
...
const buildLayer = (contents: Record<string, string>) =>
  makeTomlLoaderLive(HOME).pipe(
    Layer.provide(MemoryFileSystem.layerWith(contents)),
    Layer.provide(PathModule.layer),
  )

Feature('mergeByOverride kernel — pure precedence contract')
  .body(({ scenario }) => {
    scenario(
      'Should merge both keys when keys are disjoint',
      Gherkin.Do.pipe(
        Given('two layers with disjoint keys')('layers', () => Effect.succeed([{ a: ['1'] }, { b: ['2'] }])),
        When('mergeByOverride folds them')('out', (s) => Effect.succeed(mergeByOverride(s.layers))),
        Then('both keys appear')((s) =>
          Effect.sync(() => {
            expect(s.out).toEqual({ a: ['1'], b: ['2'] })
          })
        ),
      ),
    )
    ...
    scenario(
      'Should let later layer win when same key is set in both',
      Gherkin.Do.pipe(
        Given('two layers that set the same key')(
          'layers',
          () => Effect.succeed([{ a: ['user'] }, { a: ['project'] }]),
        ),
        When('mergeByOverride folds them')('out', (s) => Effect.succeed(mergeByOverride(s.layers))),
        Then('the later value wins')((s) =>
          Effect.sync(() => {
            expect(s.out).toEqual({ a: ['project'] })
          })
        ),
      ),
    )
    ...
    scenario(
      'Should let final layer win across three layers',
      Gherkin.Do.pipe(
        Given('three layers each setting the same key')(
          'layers',
          () => Effect.succeed([{ a: ['user'] }, { a: ['project'] }, { a: ['local'] }]),
        ),
        When('mergeByOverride folds them')('out', (s) => Effect.succeed(mergeByOverride(s.layers))),
        Then('the final value wins')((s) =>
          Effect.sync(() => {
            expect(s.out).toEqual({ a: ['local'] })
          })
        ),
      ),
    )
    ...
    scenario(
      'Should return empty when no layers are given',
      Gherkin.Do.pipe(
        Given('no layers')('layers', () => Effect.succeed([] as ReadonlyArray<Record<string, readonly string[]>>)),
        When('mergeByOverride folds them')('out', (s) => Effect.succeed(mergeByOverride(s.layers))),
        Then('the result is empty')((s) =>
          Effect.sync(() => {
    ...
    scenario(
      'Should keep earlier value when later layer omits a key',
      Gherkin.Do.pipe(
        Given('first layer sets a and b, second layer sets only b')(
          'layers',
          () => Effect.succeed([{ a: ['user'], b: ['user-b'] }, { b: ['project-b'] }]),
        ),
        When('mergeByOverride folds them')('out', (s) => Effect.succeed(mergeByOverride(s.layers))),
        Then('a inherits from the first layer, b is overwritten')((s) =>
          Effect.sync(() => {
            expect(s.out).toEqual({ a: ['user'], b: ['project-b'] })
          })
        ),
      ),
    )
    ...
    scenario(
      'Should replace array whole when later layer overrides array',
      Gherkin.Do.pipe(
        Given('first layer sets a three-element array, second layer sets one element')(
          'layers',
          () => Effect.succeed([{ plugins: ['a', 'b', 'c'] }, { plugins: ['x'] }]),
        ),
        When('mergeByOverride folds them')('out', (s) => Effect.succeed(mergeByOverride(s.layers))),
        Then('the second layer replaces the array whole')((s) =>
          Effect.sync(() => {
            expect(s.out).toEqual({ plugins: ['x'] })
          })
        ),
      ),
    )
    ...
    scenario(
      'Should preserve insertion order of keys',
      Gherkin.Do.pipe(
        Given('three layers with out-of-order keys')(
          'layers',
          () => Effect.succeed([{ z: ['1'] }, { a: ['2'] }, { m: ['3'] }]),
        ),
        When('mergeByOverride folds them')('out', (s) => Effect.succeed(mergeByOverride(s.layers))),
        Then('the keys appear in first-seen order')((s) =>
          Effect.sync(() => {
            expect(Object.keys(s.out)).toEqual(['z', 'a', 'm'])
          })
        ),
      ),
    )
...
Feature('TomlLoader — three-layer config with per-key override')
  .body(({ scenario }) => {
    scenario(
    ...
    scenario(
      ...
      Gherkin.Do.pipe(
        ...
        When('TomlLoader.load is called twice')('result', (s) =>
          Effect.gen(function*() {
            const loader = yield* TomlLoader
