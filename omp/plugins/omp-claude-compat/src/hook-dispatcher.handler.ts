...
export default function hookDispatcherExtension(pi: ExtensionAPI): void {
  ...
  const runtime = ManagedRuntime.make(nodeLayer.pipe(Layer.provideMerge(telLayer)))

  const runSafe = async <A, E>(
    effect: Effect.Effect<A, E, CommandExecutor | FileSystem | HookDispatcherExecutorDeps>,
  ) => {
  ...
  pi.on(
    'session_compact',
    (_event: { type: string }, ctx: ExtensionContext) =>
      runSafe(Effect.gen(function*() {
        const settings = yield* loadSettings(ctx.cwd)
        if (!settings) return undefined
