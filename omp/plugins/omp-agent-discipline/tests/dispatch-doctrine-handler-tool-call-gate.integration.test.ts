...
import { DOCTRINE_KERNEL } from '@systemfsoftware/omp-agent-discipline'
import type { DispatchDoctrineExtension as _DispatchDoctrineExtension } from '@systemfsoftware/omp-agent-discipline'
        ...
        Given('a fresh session and the gate enabled (real tmp dir)')(
          ...
          () =>
            Effect.promise(async () => {
              ...
              vi.resetModules()
              const mod = await import('@systemfsoftware/omp-agent-discipline')
              const { runSafe } = await import('@systemfsoftware/omp-agent-discipline')
