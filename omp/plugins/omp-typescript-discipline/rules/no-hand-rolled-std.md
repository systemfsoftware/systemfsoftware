---
description: Prefer Effect / JSR @std/* over hand-rolled encoding, async, bytes, and path primitives
condition: '(btoa\s*\(|atob\s*\(|Buffer\.from\([^)]*base64|\.toString\([^)]*base64|charCodeAt[^\n]*toString\(16\)|new\s+Promise\s*\(\s*\w+\s*=>\s*setTimeout|clearTimeout\s*\(|for\s*\([^)]*\)\s*\{[^\}]*setTimeout|Uint8Array[\s\S]*?for\s*\(|\.split\(|\+\s*[\x22\x27]\/[\x22\x27])'
scope:
