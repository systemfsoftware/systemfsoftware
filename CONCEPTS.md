...
One sandwich, authored as a continuation chain — `Sandwich.named('order.submit')(readFn)` exposes `{decode, decide}`; `.decode(Sandwich.pure(fn))` exposes `{decide}`; `.decide(workflow)` exposes `{encode}` on a decoded chain and `{write}` on a raw chain; `.encode(Sandwich.pure(fn))` exposes `{write}`; `.write(fn)` returns the Cell intersected with a recorded `phases` tuple (`readonly ['read','decide','write']` or the five-phase tuple): read (impure), decode and decide (pure), encode (pure), write (impure) — composed into a Cell: a single function from command to response. Each step closes over the inner `run` at construction, so the phases compose in that order by construction; the author cannot author a different order, because a misordered chain omits the lawful next member and fails to compile with the missing member naming what must come next. The name is a static operation identifier, so the cell emits its trace because it was named. A **phase** is one named step — a read, a decode, a decision, an encode, or a write. One Cell is one sandwich; a site whose real order writes before it can classify is two Cells composed in the calling `Effect.gen` (or through `Cell.andThen`), with the shell owning the binding between them — a later read that needs durable state an earlier write created reads it by re-gathering, and a response that becomes the next command travels as an ordinary generator binding.
...
An impure phase's interior is not type-visible, so no count of I/O operations is claimed or enforced: a read may gather a product across its interior — bumping a counter and returning the resulting rate is one such product — and fan-in is expressed that way rather than by relaxing the chain. A pure phase is a synchronous `Result`-returning thunk, branded by `Sandwich.pure`; raw I/O is unrepresentable in a filling slot (an Effect value cannot be evaluated inside a thunk returning `Result`, and Node builtins are banned repo-wide by `no-restricted-imports`), so purity is decided by the type and the import boundary rather than by a linter.
