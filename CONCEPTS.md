...
An in-source assertion that passes every placement and matcher gate while certifying nothing. Four shapes, named at the assertion level: an expected value reconstructed by calling a helper the SUT already calls for that field, a dummy-marker self-assertion satisfying a private-target gate, a silent early return standing where a refusal assertion belongs, and a predicate no legal input can fail. The class survives relocation — moving a ceremony block into the external test tree launders it into the legal vocabulary — so eviction is purity-checked and the four shapes do not survive the move. Root cause in every shape: the expected value is drawn from the same source as the actual.

### Collection token

The single canonical `import.meta.vitest` guard statement whose textual presence makes vitest collect an in-source file — vitest's includeSource matching is a text check on that expression, comment or code, so a file calling only a laws API is never collected. A collection token is not authoring vocabulary: nothing is destructured from it, its body holds only laws calls, and it is stripped from production builds by the same define that always stripped the guard. It retires when a library vitest plugin collects laws files by import edge instead.

### Adopting package

The unit of opt-in for the in-source laws channel: one package manifest plus that package's own oxlint and vitest config. Adoption is a config-declared boundary, not a TypeScript ambient property — ambient `ImportMeta` replacement is one enforcement lever an adopter may pull, never the definition, and per-module adoption is not expressible because declaration merging is consumer-initiated and program-wide once pulled.

### Census (adopt/evict)

The sort of a repo's in-source test blocks by two keys that compose: reachability — the block's `run` binds an unexported callee, because integration tests drive what's exported — and admission — a refusal boundary or a named published contract exists. A block adopts the laws channel only when both hold; behavior-shaped blocks evict to the external test tree. The census sorts, never stretches — inventing a reserved list or a fake published key so a behavior-shaped block typechecks as laws is ceremony with extra syntax. Classified-but-unrewritten blocks stay recorded as the input for later cutovers.
