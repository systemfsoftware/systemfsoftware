...
The pure decision — one business decision as a pure function: typed command in, `Either<Decision, Error>` out, no I/O. Decision variants are `S.TaggedClass`; error variants are `S.TaggedError`. Dispatch over closed unions uses `Match.value` + `Match.tag` + `Match.exhaustive`; primitives use terminal `Match.orElse`. The `never` error channel is forbidden except for total decisions (`Allow | Block` with no other outcomes). Imported only from sibling workflows and the pure Effect data modules (`Either`, `Match`, `Schema`, `Option`, `ParseResult`) — never the Effect runtime. The gates are enforced, not documented: `Workflow.make` refuses an uninhabited (`never`) or untagged error channel at the construction site via the `Inhabited` constraint, resolving to the `UninhabitedDecision` / `UninhabitedError` / `UntaggedError` markers, and the `WorkflowBrand` phantom on `Workflow<C,D,E>` means only `make` produces a value `Cell.decide` accepts.
...
- **Cell** and **Drifted key** agree about the suffix since the 2026-08-16 retirement: the taxonomy the Cell entry replaced made the suffix the key that grants a file its powers — exactly the unverified key Drifted key names, silently reassigned by a rename. The retirement kept **Drifted key**'s reading because the disagreement was measured rather than argued — two byte-identical files, one suffixed `kernel` and one `executor`, put the purity rule loud on the first and silent on the second (`docs/solutions/architecture-patterns/label-routed-rules-are-unfalsifiable.md`). A suffix therefore describes a file and never scopes a check; a new rule keys on the type, the import edge, or the `Workflow.make` boundary.
