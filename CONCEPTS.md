...
No suffix keys on it, and none can. A suffix is an author's assertion about a file's contents, so a rule keyed on one runs only on files already carrying the name and stays silent on exactly the file it exists to catch — the general form is **Drifted key**. What decides whether a recording is honest is not its name but the runner's update state. Pinned to `none`, a missing or mismatched recording fails the run, and re-recording takes `vitest -u` (`--update`), which a reader can see in the command that produced the change. The runner's own failure text names no such flag, so the pointer has to live here. Left at its default the state resolves to `new` outside a forge, and a deleted recording is rewritten in place while the suite reports green: the forge was already closed by the runner's environment detection, so the whole exposure was the developer's shell and the agent's — the two run classes where an unreviewed recording actually gets committed.
...
Retired 2026-08-16, and with it the thirteen-role suffix taxonomy: no rule keys on a cell-role name, no config enumerates those roles, and a role name grants a file nothing. The retirement is narrower than "no rule keys on a filename" — the test-kind suffixes are live, `packages/lint/oxlint/plugins/testing/test-placement/src/rules/path.config.ts` enumerates the sanctioned set, and `test-suffix-outside-src.ts` keys on it. What replaced the cell-role suffix as the organizing unit is the **sandwich** — read (impure), decide (pure, inside a `Workflow.make` body), write (impure) — with the `make` boundary, not any name, deciding what the gates bind. See **Drifted key**, whose precedence this retirement makes structural: the measurement (label-routed rules silent on the violating file) is now the shipped state rather than a standing objection.
...
- **Cell** and **Drifted key** disagree about the suffix: the first makes it the key that grants a file its powers, the second names an unverified key that a rename silently reassigns. **Drifted key** takes precedence, because the disagreement was measured rather than argued — two byte-identical files, one suffixed `kernel` and one `executor`, put the purity rule loud on the first and silent on the second (`docs/solutions/architecture-patterns/label-routed-rules-are-unfalsifiable.md`). What the measurement condemns is an **elective** label: an author who declines the name escapes the rule, so its precision runs against nothing. A **compulsory** one is a different instrument — `test-suffix-outside-src.ts:18-24` refuses any test file outside `src/` whose name is not in the sanctioned set, which leaves no unlabeled member for a suffix-scoped rule to be silent on, and is why `behaviour-test-requires-gherkin`, `behaviour-exercises-use-case` and `behaviour-one-feature-per-file` may scope on `.integration.test.ts`. So a suffix describes a file, a suffix may not be **minted** to scope a check, and a new rule keys on the type, the import edge, or the `Workflow.make` boundary.
