...
The only hand-written assertion form permitted inside an in-source `if (import.meta.vitest)` block: `expect(...).toMatchInlineSnapshot(...)` whose expected content is authored from the case's stated intent and then verified by the run. An empty placeholder is a lint error — content is never captured at commit time, because capturing re-blesses whatever the code currently returns. The form forces the expected value to be a literal, so the computed-expectation tautology shape has no syntax to live in. A block may cover only bindings its own module does not export. Enforced by the `in-source-test-snapshot-only` and `in-source-test-targets-private` oxlint rules; the generated schema-law channel (`ruleOfSchemas`) is exempt. See **Property cell** for where property tests live instead.
...
A property test granted to a pure decision, sitting beside it under a matching name. The grant is deliberately narrow — most code carries no authored property test of its own and is covered at composition altitude instead — and each grant has a specific reason to be there. The grant has exactly two homes: the colocated `<stem>.workflow.property.test.ts` beside the workflow it covers, and the generated schema-law channel — reached by registering a codec with `ruleOfSchemas(...)`, either inside an in-source block (which the snapshot-only rule exempts) or through the generated `schema-laws.test.ts` entry point. An in-source block never holds one — in-source testing is the **Snapshot spec** channel, and a hand-written property construct there is a lint error.
