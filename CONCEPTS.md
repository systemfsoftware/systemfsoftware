...
The pure decision — one business decision as a pure function: typed command in, `Result<Decision, Error>` out, no I/O. `Workflow.make({ command, decision, error, decide })` declares all three channels as schemas: decision variants are `S.TaggedClass`, error variants are `S.TaggedError`, and `error: Schema.Never` declares a decision that cannot fail. A decision is either one exclusive outcome or a list of events (`decision: Schema.Array(EventUnion)`). Dispatch over closed unions uses `Match.value` + `Match.tag` + `Match.exhaustive`; primitives use terminal `Match.orElse`. The decider references only its own module's values and the sealed pure Effect surface (`make-body-purity`) — never the Effect runtime and never another local module. The gates are enforced, not documented: `Workflow.make` refuses, at the construction site, a decision that is not a tagged union (`UntaggedDecision`), variants that do not share one TypeId (`UnsharedTypeId`), an untagged error (`UntaggedError`), and an exclusive decision with fewer than two outcomes across its success and error channels together (`SingleVariantDecision`); the `WorkflowBrand` phantom means only `make` produces a value a sandwich's `.decide` accepts.

Decision variants share one family brand (`Decision family brand`). One success variant is a workflow only when its error channel is inhabited; a single outcome that cannot fail is a plain function inside its owning module, not a workflow. The decision variants and the `decision` / `error` schemas live in the workflow module, because the decider may construct only what its own module declares; `effect-schema-vite` derives their round-trip laws from every schema exported under `src/`, so no hand-written codec test exists for them. Every variant keeps at least one producer in the decider body — a union member nobody can produce is invented, not decided.
...
One sandwich, authored as a continuation chain — `Sandwich.named('order.submit')(readFn)` exposes `.decide(workflow)`; `.decide(workflow)` exposes `.write(handlers)`; `.write(handlers)` returns the Cell intersected with the recorded `phases` tuple (`readonly ['read', 'decode', 'decide', 'encode', 'write']`): read (impure, returning command `Encoded`), decode (pure, derived via command schema), decide (pure, inside `Workflow.make`), encode (pure, derived via decision and error schemas), write (impure, tag-keyed exhaustive handler record) — composed into a Cell: a single function from input to response. Each step closes over the inner `run` at construction, so the phases compose in that order by construction; the author cannot author a different order, skip decode/encode, or omit a decision/error tag from the write handlers. The name is a static operation identifier, so the cell emits its trace because it was named. A **phase** is one named step — read, decode, decide, encode, write. One Cell is one sandwich; repeated execution or polling is composed via `Effect.repeat`, `Effect.retry`, or `Schedule` over `cell.run`, and sequencing across cells is composed in the calling `Effect.gen` (or through `Cell.andThen`), with the shell owning the binding between them.

### Grain table

The three-way classification of every operation on a Cell or runtime by what it does with requirements: **wiring closure** eliminates `R` by building the service graph (`ManagedRuntime.make`, `Layer.provide`, `Cell.provideContext`), **interpretation** starts a fiber over that graph (`runtime.runPromise`, `Layer.launch`), and **work** leaves `R` open as arrow application (`cell.run(input)`).
...
An impure phase's interior is not type-visible, so no count of I/O operations is claimed or enforced: a read may gather a product across its interior — bumping a counter and returning the resulting rate is one such product — and fan-in is expressed that way rather than by relaxing the chain. Decode and encode are derived from the workflow's declared schemas (`Schema.decodeUnknownResult` and `Schema.encodeResult`); raw I/O is unrepresentable in the pure core, so purity is decided by the schemas, the workflow constructor, and the import boundary rather than by author-supplied callback functions.
...
A refinement filter whose arbitrary values are generated by rejection sampling (discarding) rather than constructive synthesis, explicitly declared with an `arbitrary: { residual: true }` annotation on the filter. Effect Schema v4 rc.116 provides per-value constructive generation via `arbitraryConstraint` (range, length, patterns, integer), but provides no compiler hook to synthesize cross-field constraints (such as `low <= high`) constructively on a struct. Dense cross-field predicates (accepting a large share of draws, typically ≥20%) generate reliably under `maxDiscards` via rejection sampling; declaring them with the residual marker provides the justification a textual lint rule (`schema-filter-constructive-generation`) requires to distinguish deliberate rejection sampling from unexamined discard-based generation. Sparse predicates that exhaust `maxDiscards` must instead move to `Schema.declare` with `toCodecArbitrary`.
