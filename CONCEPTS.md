...
The success channel holds at least two decision variants sharing one family brand (`Decision family brand`); a single-outcome function is a plain function inside its owning module, not a workflow; only *.workflow.ts and *.schema.ts are separate files. Every variant keeps at least one producer in the decider body — a union member nobody can produce is invented, not decided.
