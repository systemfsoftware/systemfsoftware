...
- **Tests:** vitest property suite `src/__tests__/lint-edit.workflow.property.test.ts` plus in-source properties in `src/verdict.ts` (gate: `corepack pnpm exec vitest run` exits 0, 14 tests); mutation gate is CI-only on `src/*.workflow.ts` at 100% (gate: `.github/workflows/mutation.yml`).
