...
import { decideNodeFate, type NodeFate } from './node-lifetime.observer.js'
...
const atomKey = <A>(atom: Atom.Atom<A>): Atom.Atom<A> | string =>
  Serializable.SerializableTypeId in atom ? (atom as Atom.Serializable<any>)[Serializable.SerializableTypeId].key : atom
...
class RegistryImpl implements Registry.Registry {
  ...
  readonly timeoutBuckets = new Map<number, readonly [nodes: Set<Node<any>>, cancel: () => void]>()
  ...
  ensureNode<A>(atom: Atom.Atom<A>): Node<A> {
    ...
    if (
      Serializable.isSerializable(atom) && this.preloadedSerializable.has(atom[Serializable.SerializableTypeId].key)
    ) {
      const encoded = this.preloadedSerializable.get(atom[Serializable.SerializableTypeId].key)
      this.preloadedSerializable.delete(atom[Serializable.SerializableTypeId].key)
      const decoded = atom[Serializable.SerializableTypeId].decode(encoded)
  ...
  createNode<A>(atom: Atom.Atom<A>): Node<A> {
    if (this.disposed) {
      throw new Error(`Cannot access Atom ${String(atom)}: registry is disposed`)
    }
  ...
  removeNode(node: Node<any>): void {
    const fate = node.nodeFate
    if (fate._tag === 'RemoveAfterTtl') {
      this.setNodeTimeout(node)
    } else if (fate._tag === 'RemoveNow') {
  ...
  removeNodeTimeout(node: Node<any>): void {
    ...
    const [nodes, cancel] = this.timeoutBuckets.get(bucket)!
    nodes.delete(node)
    if (nodes.size === 0) {
      cancel()
...
class Node<A> {
  ...
  get nodeFate(): NodeFate {
    return decideNodeFate({
      keepAlive: this.atom.keepAlive,
      listenerCount: this.listeners.size,
      childCount: this.children.length,
      isLive: this.state !== 0,
      isWaiting: Result.isResult(this._value) && Result.isInitial(this._value) && this._value.waiting,
  ...
  get canBeRemoved(): boolean {
    return this.nodeFate._tag !== 'Alive'
  }
...
const disposedError = (atom: Atom.Atom<any>): Error => new Error(`Cannot use context of disposed Atom: ${String(atom)}`)
...
const LifetimeProto: Omit<Lifetime<any>, 'node' | 'finalizers' | 'disposed' | 'isFn'> = {
  ...
  result<A, E>(this: Lifetime<any>, atom: Atom.Atom<Result.Result<A, E>>, options?: {
    ...
    return Result.match(result, {
      onInitial: () => Effect.never,
      onFailure: (failure) => Exit.failCause(failure.cause),
  ...
  streamResult<A, E>(this: Lifetime<any>, atom: Atom.Atom<Result.Result<A, E>>, options?: {
    ...
    return pipe(
      ...
      Stream.flatMap((result) =>
        Result.isSuccess(result) ? Stream.succeed(result.value) : Stream.failCause(result.cause)
      ),
    )
  },
