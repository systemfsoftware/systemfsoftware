...
/**
 * The host side of the run-event sink (U13): the Effect stream that turns
 ...
 * CLI composition root (`stryker-cli.ts`) forks the drain before the run,
 * resolves mode once at the edge, and writes the terminal line from an
 * `onExit` finalizer so it lands on success, failure and interruption alike.
 */
 ...
 */
export interface StdoutWriteError {
  ...
  return Sink.fromPush<string, StdoutWriteError, void, never, never>(
    Effect.sync(() => {
      ...
      const writeChunk = (chunk: Chunk.Chunk<string>): Effect.Effect<void, StdoutWriteError, never> =>
        Chunk.reduce(
          chunk,
          Effect.void as Effect.Effect<void, StdoutWriteError, never>,
...
export function createRunEventStream(resolved: ResolvedMode): RunEventStream {
  ...
  const framed = Stream.merge(eventStream, tickStream, { haltStrategy: 'either' }).pipe(
    ...
    // the writer this replaces terminated every line, so a byte-level
