...
import { childProcessAsPromised, Immutable } from '@stryker-mutator/util'

import { DEFAULT_CONFIG_FILE_NAMES, SUPPORTED_CONFIG_FILE_NAMES } from '../config/index.js'
...
export class StrykerConfigWriter {
  ...
  private async checkIfConfigFileExists(file: string) {
    if (await fileUtils.exists(file)) {
      const msg =
        `Stryker config file "${file}" already exists in the current directory. Please remove it and try again.`
      this.log.error(msg)
      throw new Error(msg)
    }
  }
  ...
  public write(
    ...
    const configObject: Partial<StrykerOptions> & { _comment: string } = {
      ...
      testRunner_comment:
        `Take a look at ${homepageOfSelectedTestRunner} for information about the ${selectedTestRunner.name} plugin.`,
  ...
  public async writeCustomInitializer(
    ...
    const config = {
      _comment:
        `This config was generated using 'stryker init'. Please see the guide for more information: ${initializerConfig.guideUrl}`,
