...
import { type NodePath as IgnorerNodePath } from '@systemfsoftware/stryker-js/Ignorer'
...
import { propertyPath, type StrykerOptions, strykerReportBugUrl } from '@systemfsoftware/stryker-js/Options'
...
/** An ignorer as the instrumenter consumes it: the reason to skip a node, or null. */
export type InstrumentIgnorer = (node: IgnorerNodePath, fileName: string) => string | null

export interface TransformerOptions extends MutatorOptions {
  ignorers: InstrumentIgnorer[]
}
...
export const angularIgnorer: InstrumentIgnorer = (node) => Option.getOrNull(shouldIgnore(node))
...
export const transformScript: AstTransformer<ScriptFormat> = (
  ...
  function ignorerReason(view: IgnorerNodePath): Option.Option<string> {
    return options.ignorers.reduce(
      (reason, ignorer) => Option.orElse(reason, () => Option.fromNullishOr(ignorer(view, originFileName))),
      Option.none<string>(),
    )
  }
