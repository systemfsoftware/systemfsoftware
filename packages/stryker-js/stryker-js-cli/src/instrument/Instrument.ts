...
import { coreFormatOf, createParser } from './Parser.js'
import type { CoreFormat, ParserContribution } from './Parser.js'
import { print } from './Printer.js'
import { type Ast, isAst, type ScriptAst } from './Syntax.js'
import { createMutantCollector, transform } from './Transformer.js'
import type { InstrumentIgnorer, TransformerOptions } from './Transformer.js'
...
export async function disableTypeChecks(
  ...
  const coreFormat = coreFormatOf(file.name)
  if (coreFormat !== undefined && JS_OR_TS_FORMATS[coreFormat] === true && !tsDirectiveLikeRegEx.test(file.content)) {
    return { ...file, content: prefixWithNoCheck(file.content) }
  }
  if (!parser.knowsFile(file.name)) {
    return file
  }
  return withDisabledTypeChecking(file, await parser(file.content, file.name))
}
...
const JS_OR_TS_FORMATS: Readonly<Partial<Record<CoreFormat, true>>> = { js: true, ts: true }
...
function isInstrumentIgnorer(value: unknown): value is InstrumentIgnorer {
  return typeof value === 'function'
}

function isParserContribution(value: unknown): value is ParserContribution {
  return (
    Predicate.hasProperty(value, 'extensions') &&
    Array.isArray(value['extensions']) &&
    Predicate.hasProperty(value, 'parse') &&
...
function toTransformerOptions(options: InstrumenterOptions): TransformerOptions {
  const base: TransformerOptions = {
    ...
    ignorers: options.ignorers.filter(isInstrumentIgnorer),
  }
...
const readCollected = (command: InstrumentCommand): Effect.Effect<Collected, InstrumentError> =>
  Effect.gen(function*() {
    ...
    const parse = createParser(command.options.parsers.filter(isParserContribution))
