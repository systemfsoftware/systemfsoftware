...
import type { MutantStatus, Position } from '@systemfsoftware/stryker-js/Mutant'
...
import { reporterFailure } from './reporter-failure.js'
...
function getEmojiForStatus(status: MutantStatus): string {
  switch (status) {
    case 'Killed':
      return '✅'
    case 'NoCoverage':
...
function mutantLabel(status: MutantStatus, allowEmojis: boolean): string {
  if (allowEmojis) {
