  ...
  scenario(
    ...
    Gherkin.Do.pipe(
      ...
      Then('each batch is typechecked on its own')((s: {
        ...
        checkExpect(Equal.equals(batchSet(s.fixture.checkIdSets), batchSet([['a', 'b'], ['c']]))).toBe(true)
        checkExpect(
          Equal.equals(
  ...
  scenario(
    ...
    Gherkin.Do.pipe(
      ...
      Then('no typecheck runs')((s: { fixture: Recording }) => {
        checkExpect(Equal.equals(batchSet(s.fixture.checkIdSets), batchSet([]))).toBe(true)
      }),
    ),
  )
