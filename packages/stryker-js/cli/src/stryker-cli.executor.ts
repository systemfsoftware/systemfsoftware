...
import type { Mutant, PartialStrykerOptions, StrykerOptions } from '@stryker-mutator/api/core'
...
function remediationFor(exit: Exit.Exit<unknown, unknown>, code: number): string {
  ...
  if (value !== undefined) {
    ...
    if (Schema.is(SurvivorsRejection)(value)) {
...
function resolveCliExitCode(exit: Exit.Exit<unknown, unknown>): number {
  ...
  if (Option.isSome(failure)) {
    ...
    if (Schema.is(SurvivorsRejection)(failure.value)) {
