 ...
 * for that job because it also exports `BroadcastReporter` and `StrictReporter`,
 * which do not belong in the published surface the loader reads — so the
 * enumerated entry is this file, not the barrel (the plan's U7 ruling, same
 * shape as `utils/exit-classification.ts`).
 */
