...
Feature('Workflow.make boundary — the inverted mutation-population selector')
  .body(({ scenario }) => {
    ...
    scenario(
      'The workflow boundary entrypoint declares an ignorer',
      Gherkin.Do.pipe(
        ...
        Then('it is an ignorer named for the workflow boundary, carrying a factory')((s) =>
          Effect.sync(() => {
            expect(s.plugin).toMatchObject({ kind: 'Ignorer', name: 'workflow-make-boundary' })
            expect(typeof s.plugin?.make).toBe('function')
          })
        ),
      ),
    )
