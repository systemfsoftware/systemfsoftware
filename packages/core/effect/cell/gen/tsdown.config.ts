...
// `Gen.Bag` is TS2749 ("refers to a value, but is being used as a type") and every
// consumer's bag collapses to unknown. Measured on the source package: with the rollup as the
// types entry, `test:types` and downstream typechecks fail. api-extractor still runs for the
// API report in `etc/`, which is the gate that matters; only its rollup is disabled.
