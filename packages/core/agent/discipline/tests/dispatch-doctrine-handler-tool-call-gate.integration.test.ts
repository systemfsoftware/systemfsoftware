...
import type { DispatchDoctrineExtension as _DispatchDoctrineExtension } from '@systemfsoftware/agent-discipline'
        ...
        Given('a fresh session and the gate enabled (real tmp dir)')(
          ...
          () =>
            Effect.promise(async () => {
              ...
              const mod = await import('@systemfsoftware/agent-discipline')
            ...
            Effect.promise(async () => {
              try {
                ...
                for (let i = 0; i < 50; i++) {
                  ...
                  const _floodAppLayer = Layer.mergeAll(
                    _floodNodeLayer,
                    HarnessPolicyLive.pipe(Layer.provideMerge(_floodNodeLayer)),
                  )
