...
rules:
  ...
  - id: T4
    ...
    check: the encoded-side pattern enforces byte-pair if the type side is a byte type
```
...
- id: A2
  ...
  check: the union uses `boundedUnion` iff any member references the union itself
    through a non-array field
```
