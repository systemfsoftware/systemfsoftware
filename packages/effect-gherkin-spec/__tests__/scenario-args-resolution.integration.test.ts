 ...
 * proves that `Spec.resolveScenarioArgs` returns the right defensive pipeline in
...
Feature('Scenario registration — argument resolution').body(({ scenario }) => {
  scenario(
    ...
    Effect.gen(function*() {
      const { pipeline } = Spec.resolveScenarioArgs<never>(void 0, void 0)
      const result = yield* Effect.either(pipeline)
      expect(result).toEqual(
        Either.left(
          new Spec.StepError({ keyword: 'scenario', text: 'pipeline or options required', cause: void 0 }),
        ),
      )
    }),
  )
  ...
  scenario(
    ...
    Effect.gen(function*() {
      const { pipeline } = Spec.resolveScenarioArgs<never>({ layer: Layer.empty }, void 0)
      const result = yield* Effect.either(pipeline)
      expect(result).toEqual(
        Either.left(
          new Spec.StepError({
