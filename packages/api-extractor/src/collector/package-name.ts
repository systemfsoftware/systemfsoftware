...
export const parsePackageName = (
  ...
  return Match.value(result.error).pipe(
    Match.when('', () => Result.succeed({ scope: result.scope, unscopedName: result.unscopedName })),
    Match.orElse((error) => Result.fail(makePackageNameParseError(packageName, error))),
  )
}
