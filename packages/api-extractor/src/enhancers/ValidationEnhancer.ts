  ...
  public static analyze(collector: Collector): void {
    ...
    for (const entity of collector.entities) {
      if (
        !(
          entity.consumable ||
          collector.extractorConfig.apiReport.includeForgottenExports ||
          collector.extractorConfig.docModel.includeForgottenExports
        )
      ) {
        continue
      }
...
function _checkReferences(
  ...
  for (const referencedEntity of astDeclaration.referencedAstEntities) {
    ...
    if (collectorEntity && collectorEntity.consumable) {
      ...
      const entryPointFilename: string = collector.workingPackage.entryPointSourceFile.fileName.split('/').pop() ?? ''
...
function _isEcmaScriptSymbol(astSymbol: AstSymbol): boolean {
  ...
  const astDeclaration: AstDeclaration | undefined = astSymbol.astDeclarations[0]
  if (!astDeclaration) {
    return false
  }
