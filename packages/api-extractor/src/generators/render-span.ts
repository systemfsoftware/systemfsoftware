...
const sortedSubsetOf = (
  ...
  const modification = SpanPlan.modificationOf(plan, tree)
  return Option.some(tree.children).pipe(
    Option.filter(() => !modification.omitChildren),
    Option.filter(() => modification.sortChildren),
...
const writeModifiedSpan = (
  ...
  const modification = SpanPlan.modificationOf(plan, tree)
  const prefix = Option.getOrElse(modification.prefix, () => tree.prefix)
  const suffix = Option.getOrElse(modification.suffix, () => tree.suffix)
  const scope = modification.indentDocComment
