...
/** A configuration record as the merge fold sees it: a JSON object with replaceable keys. */
export type MutableJsonRecord = Record<string, Schema.Json>

const isConfigRecord = (u: Schema.Json | undefined): u is MutableJsonRecord =>
...
/** The base carried keys the derived record merged key by key; nothing was replaced wholesale. */
...
/** The base was empty, so the derived record is the result unchanged. */
...
/** One merge step: a base record and the derived record folded onto it. */
...
/** Both sides are objects with a key to compare: recurse; anything else replaces. */
...
/**
 * The merge: objects merge key by key, and every other value — including arrays — replaces the
 * base value wholesale, matching upstream `ExtractorConfig.ts`.
 */
