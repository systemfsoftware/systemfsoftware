...
export class TypeScriptHelpers extends Pipeable.Class {
  ...
  public static isAmbient(symbol: ts.Symbol, typeChecker: ts.TypeChecker): boolean {
    ...
    if (followedSymbol.declarations && followedSymbol.declarations.length > 0) {
      const firstDeclaration: ts.Declaration | undefined = followedSymbol.declarations[0]
      if (!firstDeclaration) {
        return false
      }
