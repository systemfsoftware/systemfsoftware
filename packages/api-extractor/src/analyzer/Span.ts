...
import { sortBy } from '../collector/sort.js'
import { invariant } from '../utils/invariant.js'
import { convertToLf, truncateWithEllipsis } from './text.js'

import { IndentedWriter } from './indented-writer.js'
...
export class Span extends Pipeable.Class {
      ...
      if (previousChildSpan) {
        if (previousChildSpan.endIndex < childSpan.startIndex) {
          ...
          while (separatorRecipient.children.length > 0) {
            const lastChild: Span | undefined = separatorRecipient.children[separatorRecipient.children.length - 1]
            if (!lastChild || lastChild.endIndex !== separatorRecipient.endIndex) {
              break
            }
   ...
   */
  public get suffix(): string {
    const lastChild = this.children.length > 0 ? this.children[this.children.length - 1] : undefined
    if (lastChild) {
      // Everything after the last child
      return this.#getSubstring(lastChild.endIndex, this.endIndex)
    } else {
      return ''
    }
  }
  ...
  public getLastInnerSeparator(): string {
    ...
    const lastChild = this.children.length > 0 ? this.children[this.children.length - 1] : undefined
    if (lastChild) {
      return lastChild.getLastInnerSeparator()
    }
    return ''
  }
  ...
  #writeModifiedText(options: IWriteModifiedTextOptions): void {
    ...
    if (sortedSubset) {
      ...
      sortBy(sortedSubset, (x) => x.modification.sortKey)
      ...
      for (let index: number = 0; index < this.children.length; ++index) {
        ...
        if (childItem.modification.sortKey === undefined) {
          ...
          const nextItem = sortedSubset[sortedSubsetIndex++]
          if (!nextItem) {
            break
          }
          current = nextItem
      ...
      if (!this.modification.omitChildren) {
        if (options.separatorOverride !== undefined) {
          ...
          for (let i: number = 0; i < childrenLength; ++i) {
            const child: Span | undefined = this.children[i]
            if (!child) {
              continue
            }
...
const formatTrimmedText = (text: string): string => truncateWithEllipsis(convertToLf(text), 100)
...
if (import.meta.vitest !== void 0) {
  ...
  it.prop(
    '∀s_Trimmed_≤Bound',
    [Schema.String],
    ([str]) => formatTrimmedText(str).length <= 100,
  )
}
