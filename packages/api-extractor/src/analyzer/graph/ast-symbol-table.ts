...
import { lineAndColumnOf } from '../position.js'
...
import { type AstModule, isExternalModule } from './ast-module.js'
...
const lineOf = (node: ts.Node): number => lineAndColumnOf(node.getSourceFile(), node.getStart()).line
  ...
  Effect.flatMap(
    ...
    (nominalAnalysis) =>
      Option.match(nominalAnalysis, {
        ...
        onSome: (nominal) =>
          Effect.flatMap(
            unsupportedDeclarationsOf(options, nominal),
            () =>
              Effect.flatMap(parentAstSymbolOf(ref, options, arbitraryDeclaration, nominal), (parentAstSymbol) =>
                Effect.flatMap(storeAstSymbol(ref, options, nominal, parentAstSymbol), (astSymbol) =>
  ...
  Effect.flatMap(
    ...
    (graph) =>
      Option.match(HashMap.get(graph.symbols, getSymbolId(options.followedSymbol)), {
        ...
        onNone: () => createAstSymbol(ref, options, arbitraryDeclaration),
      }),
  )
      ...
      Option.match(Option.fromUndefinedOr(TypeScriptHelpers.tryGetADeclaration(options.followedSymbol)), {
        ...
        onSome: (declaration) =>
          Match.value(symbolGateOf(graph, options.followedSymbol, declaration)).pipe(
            ...
            Match.when('Proceed', () => fetchOrCreateAstSymbol(ref, options, declaration)),
            Match.exhaustive,
          ),
      }),
  ))
