...
export class AstSymbolTable extends Pipeable.Class {
   ...
   *
   * ```ts
   * class X extends Pipeable.Class {
  ...
  public static getLocalNameForSymbol(symbol: ts.Symbol): string {
    ...
    for (const declaration of symbol.declarations || []) {
      // Handle cases such as "export default class X extends Pipeable.Class { }" where the symbol name is "default"
      ...
      //  }
      //
      //  class C extends Pipeable.Class {
      //    public [X.Y.z](): void { }
      //  }
      //
      if (isUniqueSymbol) {
  ...
  #fetchAstSymbol(options: IFetchAstSymbolOptions): AstSymbol | undefined {
    ...
    if (!astSymbol) {
      ...
      if (!nominalAnalysis) {
        ...
        if (arbitraryDeclaration) {
          const arbitraryParentDeclaration: ts.Node | undefined = this.#tryFindFirstAstDeclarationParent(
