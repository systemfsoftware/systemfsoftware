 ...
 */
export class ResolverFailure extends Pipeable.Class {
  /*
 ...
 */
export class AstReferenceResolver extends Pipeable.Class {
  ...
  public resolve(declarationReference: tsdoc.DocDeclarationReference): AstDeclaration | ResolverFailure {
    ...
    const rootMemberReference: tsdoc.DocMemberReference | undefined = declarationReference.memberReferences[0]
    if (rootMemberReference === undefined) {
      return new ResolverFailure('Package references are not supported')
    }
    ...
    for (let index: number = 1; index < declarationReference.memberReferences.length; ++index) {
      const memberReference: tsdoc.DocMemberReference | undefined = declarationReference.memberReferences[index]
  ...
  #selectDeclaration(
    ...
    if (memberSelector === undefined) {
      if (astDeclarations.length === 1) {
        return astDeclarations[0] ?? new ResolverFailure('No declaration found')
      } else {
        // If we found multiple matches, but the extra ones are all ancillary declarations,
        // then return the main declaration.
        const nonAncillaryMatch: AstDeclaration | undefined = this.#tryDisambiguateAncillaryMatches(astDeclarations)
  ...
  #selectUsingSystemSelector(
    ...
    return matches[0] ?? new ResolverFailure('No matching declaration')
  }
