...
import { AstImport, AstImportKind, type IAstImportOptions } from './AstImport.js'
...
export class ExportAnalyzer extends Pipeable.Class {
  ...
  public fetchAstModuleFromSourceFile(
    ...
    if (!astModule) {
      ...
      const externalModulePath: string | undefined = moduleReference !== undefined && isExternal
        ? moduleReference.moduleSpecifier
        : undefined
  ...
  #getModuleSymbolFromSourceFile(
    ...
    throw invariant('Unable to determine module for: ' + sourceFile.fileName)
  }
  ...
  #tryMatchExportDeclaration(
    ...
    if (exportDeclaration) {
      ...
      if (exportDeclaration.moduleSpecifier && exportName !== undefined) {
  ...
  #fetchSpecifierAstModule(
    ...
    const moduleSpecifier: string = this.#getModuleSpecifier(importOrExportDeclaration)
    const mode: ts.ModuleKind.CommonJS | ts.ModuleKind.ESNext | undefined = importOrExportDeclaration.moduleSpecifier &&
