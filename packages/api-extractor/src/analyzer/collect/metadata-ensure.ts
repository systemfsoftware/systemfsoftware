...
import { astSymbolOfId, nodeValueOf } from '../graph/analysis-graph.js'
...
const apiItemsNotBuilt = (
  ...
  !HashMap.has(collected.apiItemMetadata, mainDeclarationId) &&
  !HashMap.has(collected.apiItemMetadata, ancillaryDeclarationId)

const bothEligible = (sameSymbol: boolean, fresh: boolean): boolean => sameSymbol && fresh
...
const calculateDeclarationMetadataForDeclarations = (
  ...
  const parsed = Arr.reduce(
    ...
    (current, declarationId) => {
      const parsedPair = parseTsdocForDeclaration(parser, current.messageLog, graph, declarationId)
      return {
        ...current,
