...
import { isConfigRecord } from '../config/config-record.js'
...
const preservesDeeperPaths = (
  ...
  if (!isConfigRecord(base) || !isConfigRecord(derived)) return Equal.equals(merged, derived)
  if (!isConfigRecord(merged)) return false
