  ...
  try {
    ...
    for (let key in opts) {
      ...
      if (key === 'ignoreRules') {
        ...
        if (invalid) {
          program.error(
            `error: config option 'ignoreRules' argument '${invalid}' is invalid. Allowed choices are ${
              Object.values(
                problemFlags,
              ).join(', ')
            }.`,
      ...
      if (key === 'profile') {
        ...
        if (!(value in profiles)) {
          program.error(
            `error: config option 'profile' argument '${value}' is invalid. Allowed choices are ${
              Object.keys(
                profiles,
              ).join(', ')
            }.`,
