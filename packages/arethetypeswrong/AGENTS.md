...
Fork of the upstream arethetypeswrong tooling (the checker behind arethetypeswrong.github.io), republished under this org, governing `packages/arethetypeswrong/cli` and `packages/arethetypeswrong/core`.
...
- **Forked compiler-driven tooling, not Effect cell code.** Both packages are `TOOLING` entries in `scripts/guards/check-lint-coverage.mjs` ("port of arethetypeswrong, tooling"), so the cell oxlint rules do not apply here. Read that entry as a lint-baseline choice only — it says nothing about whether the packages publish. Neither package carries an `oxlint.config.ts`, and neither has a `lint` script.
