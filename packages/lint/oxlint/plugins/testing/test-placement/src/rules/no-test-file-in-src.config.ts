...
export const testFileInSrcDetail = (dir: string): Detail => ({
  ...
  fix:
    `pick the arm matching what this file exercises. A workflow law -> rename to <stem>.workflow.property.test.ts inside src/<path>/${dir}/. A kernel/policy/schema property or characterization suite -> delete it: property coverage lives at the workflow's exported contract or in generated schema laws, and an in-source block holds authored inline snapshots only. The package public surface -> move outside src/ to tests/<name>.integration.test.ts. No arm matches -> delete this file`,
})
...
export const propertyTestLocationDetail = (dir: string): Detail => ({
  ...
  fix:
    `a workflow law -> rename to <stem>.workflow.property.test.ts inside src/<path>/${dir}/. A kernel/policy/schema property suite -> delete it: in-source blocks are authored inline snapshots, never property suites. Relative imports shift one level when moving into ${dir}: ./<cell>.js -> ../<cell>.js`,
})
...
export const SCHEMA_TEST_DETAIL: Detail = {
  ...
  fix:
    'delete it. The generated laws already state round-trip identity and encode stability. What they cannot state is rejection — every input they draw comes from the arbitrary the schema itself supplies — so a refusal belongs in the schema file as an authored inline snapshot inside an if (import.meta.vitest) block, never here',
}
