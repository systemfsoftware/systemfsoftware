...
| `property-file-purity`     | Property tests live ONLY in `*.property.test.ts` files (no plain `it()`/`it.effect()` or raw `fc.assert` there), and scenario test files contain no FastCheck imports and no `it.prop` / `it.effect.prop` calls. The two never mix. |
