...
| `property-file-purity`     | Property tests live ONLY in `*.property.test.ts` files (no plain `it()`/`it.effect()` or raw `fc.assert` there), and scenario test files contain no FastCheck imports and no `it.prop` / `it.effect.prop` calls. The two never mix. A `*.snapshot.test.ts` is exempt from the FastCheck-import ban: deterministic seeded sampling IS the snapshot kind. |
