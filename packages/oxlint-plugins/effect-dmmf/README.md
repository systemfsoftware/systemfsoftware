...
A combined oxlint plugin bundling the DMMF cell quartet under one
entrypoint: the pure decision (`@systemfsoftware/oxlint-plugin-effect-workflow`),
the operation shell (`@systemfsoftware/oxlint-plugin-effect-executor`),
their property-test verification altitude
(`@systemfsoftware/oxlint-plugin-property-testing`), and the filename
taxonomy every cell is named by
(`@systemfsoftware/oxlint-plugin-cell-taxonomy`). Install one package,
register one `jsPlugin`, get all four rule sets.

This plugin defines no rules of its own. It re-exports the four sources'
...
This is equivalent to registering all four source plugins and spreading each
...
`configs.recommended.rules` is **not** every rule the four sources register —
...
`src/index.ts` merges exactly four fixed, already-tested plugins with a
