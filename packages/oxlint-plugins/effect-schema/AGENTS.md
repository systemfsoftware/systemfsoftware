...
- id: ES3
  ...
  check: `grep -oE '\b(ruleOfSchemas|equivalence|encodedSchema)\b' packages/effect-schema-law/src/rule-of-schemas.harness.ts | sort -u` lists exactly ruleOfSchemas, equivalence, encodedSchema — the three symbols `GENERATED_LAW_NAMES` holds
```
