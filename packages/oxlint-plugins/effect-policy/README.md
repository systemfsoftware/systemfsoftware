...
```
x @systemfsoftware/effect-policy(policy-no-domain-imports): rate-limit.policy.ts is forbidden.
...
These four rules make that convention executable. Every rule is inert on any file not named `*.policy.ts`.
...
| `policy-no-domain-imports`   | An import of any domain cell module — `.schema`, `.shape`, `.state`, `.workflow`, `.executor`, `.store`, `.acl`, `.handler`, `.middleware`, `.adapter`, `.service`, `.shell`, `.use-case`, `.daemon`, `.repository`. Sibling `.policy` (composition) and `.kernel` (utilities) imports pass (PO1)                                                                                                                     |
