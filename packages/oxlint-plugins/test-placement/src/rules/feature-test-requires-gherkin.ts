...
import { FEATURE_SUFFIX, FOREIGN_RUNNERS, GHERKIN_PACKAGE, RUNNER_NAMES } from './path.config.js'
...
export const featureTestRequiresGherkin = defineRule({
  meta,
  create(context: Context) {
    const basename = basenameOf(context.filename)
    return {
      Program(node: ESTree.Program) {
        if (!basename.endsWith(FEATURE_SUFFIX)) return
