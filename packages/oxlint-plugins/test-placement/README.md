...
| `behaviour-no-pure-cell-import`   | A `.integration.test.ts` must not import a pure cell (`.kernel`, `.workflow`, `.schema`, `.acl`) directly; pure cells are proven by property tests colocated in `src/`.                                                                                                        |
