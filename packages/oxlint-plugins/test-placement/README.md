...
| `src-property-test-cell`         | A property test under `src/` must be colocated with a `.workflow` or `.policy` cell. A schema carries no authored test — its laws are generated.                        |
