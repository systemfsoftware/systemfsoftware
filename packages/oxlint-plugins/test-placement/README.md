...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `no-test-file-in-src`            | Under `src/`, only `*.schema.test.ts` and `*.property.test.ts` test files are allowed.                                                                                  |
| `src-property-test-cell`         | A property test under `src/` must be colocated with a `.workflow` or `.policy` cell. A schema cell carries its laws in `*.schema.test.ts`.                              |
