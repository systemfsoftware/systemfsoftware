...
- id: ES2
  ...
  check: review — a real `*.shape.ts` in the workspace passes every rule in this package (shape-no-behaviour, shape-one-foreign-system, shape-no-anti-pattern-path) unchanged; the reviewer runs the plugin's recommended config over that file and confirms zero reports
