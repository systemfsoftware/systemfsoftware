...
const toPlatformError = (method: string) => (err: unknown): Error.PlatformError =>
  handleErrnoException('FileSystem', method)(err, [])
...
export function make(contents?: Contents, opts?: { cwd: string }): FileSystem.FileSystem {
  ...
  const openHandler = (path: string, options?: FileSystem.OpenFileOptions) => {
    ...
    ).pipe(Effect.map((memfsHandle) => makeFile(memfsHandle as unknown as FileHandle)))
  }

  const readFileFn = (path: string): Effect.Effect<Uint8Array, Error.PlatformError> =>
    Effect.tryPromise({
      try: () => nfs.promises.readFile(path) as Promise<Buffer>,
  ...
  const readDirectory = (
    ...
    Effect.tryPromise({
      try: () => nfs.promises.readdir(path) as Promise<Array<string>>,
  ...
  const pathStat = (path: string): Effect.Effect<FileSystem.File.Info, Error.PlatformError> =>
    Effect.tryPromise({
      try: () => nfs.promises.stat(path) as Promise<Stat>,
      catch: toPlatformError('stat'),
    }).pipe(Effect.map(makeFileInfo))
  ...
  const writeFile = (
    ...
    const opts: { mode?: number | undefined; flag?: string | undefined } = {}
    if (options?.mode !== undefined) opts.mode = options.mode
    if (options?.flag !== undefined) opts.flag = options.flag
    return Effect.tryPromise({
      try: () => nfs.promises.writeFile(path, data, opts as Record<string, unknown> as never),
