...
import type { Contents as SeedContents, FileHandle, Stat } from './memory-file-system.shape.js'

export class Contents extends Context.Tag(
  '@systemfsoftware/effect-memfs/memory-file-system.adapter/Contents',
...
const make = (contents?: SeedContents, opts?: { cwd: string }): FileSystem.FileSystem => {
  const cwd = opts?.cwd ?? '/'
...
export const layer: Layer.Layer<FileSystem.FileSystem, never, Contents> = Layer.effect(
  FileSystem.FileSystem,
  Effect.gen(function*() {
    const contents = yield* Contents
