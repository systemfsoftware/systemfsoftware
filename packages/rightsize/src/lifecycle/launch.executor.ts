...
import { registerDockerCleanupSync } from '../backend-docker/cli.shellout.js'
...
import { registerMsbCleanupSync } from '../backend-msb/runtime.adapter.js'
...
import { FreePortExhaustedError, FreePorts } from '../runtime/free-ports.kernel.js'
...
const toRunningHandle = (state: LaunchState): RunningHandle => {
  ...
  if (handle === undefined) {
    ...
    throw new Error('launch.executor: handleValue reached without a handle — executor bug')
  }
  return {
    handle,
