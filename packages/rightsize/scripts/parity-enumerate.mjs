...
const renderMatrix = (surface) => {
  ...
  for (const { id, title, members } of surface.sections) {
    ...
    for (const member of members) {
      ...
      const note = row.note === null || row.note === undefined ? '' : row.note
      lines.push(`| \`${esc(member)}\` | ${esc(row.status)} | ${esc(rs)} | ${esc(note)} |`)
    }
    lines.push(``)
  }
  lines.push(`---`)
...
/** The generated surface-module bytes, raw (unformatted). @param {Surface} surface @returns {string} */
const renderSurfaceModuleRaw = (surface) => {
  const header = [
  ...
  return `${header}${JSON.stringify(surface, null, 2)}\n`
}

/** The dprint-formatted surface-module bytes for writing — refuses to emit unformatted bytes (they are byte-gated against the repo format:check contract). @param {Surface} surface @returns {string} */
const renderSurfaceModule = (surface) => {
  const formatted = formatWithDprint(renderSurfaceModuleRaw(surface), SURFACE_PATH)
  if (formatted === null) {
...
const checkInstalledSurface = (installed) => {
  ...
  // Formatting-insensitive: the frozen module is imported (typed via its
  // .d.mts) and compared as data — quote style and padding are not the signal.
  if (JSON.stringify(FROZEN_SURFACE) === JSON.stringify(installed)) return 0
  process.stderr.write(
...
const checkMatrix = (surface) => {
  ...
  /** Compare content, not layout: trim line edges, compare table rows cell-by-cell, canonicalize separator cells, and normalize emphasis markers (dprint pads columns, converts `*x*` to `_x_`; the raw render does not). @param {string} text @returns {string} */
  const normalize = (text) =>
    text
      .split('\n')
      .map((line) => {
        const trimmed = line.trim()
        if (!trimmed.startsWith('|')) return trimmed.replace(/(^|[\s(])_([^_]+)_(?=$|[\s).,;:])/g, '$1*$2*')
        return trimmed
          .split('|')
