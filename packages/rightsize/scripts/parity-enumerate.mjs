...
const MAPPING = {
  'index::IntervalRetry': {
    ...
    note:
      "Upstream's bounded-retry helper class; the port-bind conflict loop retries with the same bounded semantics via Effect combinators (src/lifecycle/launch.executor.ts).",
  },
  'index::RandomUuid': {
    status: 'superseded-by',
  ...
  'socat-container::start': {
    status: 'superseded-by',
    rs: '`GenericContainer.start` (src/lifecycle/launch.executor.ts)',
...
/** The generated surface-module byte stream — the committed snapshot's exact regeneration contract. @param {Surface} surface @returns {string} */
...
const main = () => {
  ...
  if (mode === 'write') {
    ...
    fs.writeFileSync(MATRIX_PATH, renderMatrix(loadSurface(installed)))
