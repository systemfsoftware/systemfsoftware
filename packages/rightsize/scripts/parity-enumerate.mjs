 ...
 * resolved via PATH (nix develop / direnv / CI) with the repo wrapper as
 * fallback; when neither answers, the render fails loudly rather than
 * emitting bytes that `format:check` would rewrite.
 * @param {string} text @param {string} filePath @returns {string}
 */
const formatWithDprint = (text, filePath) => {
  ...
  process.stderr.write(
    'parity-enumerate: dprint is required to render the parity artifacts (they are byte-gated against the repo format:check contract)\n' +
...
/** The generated surface-module bytes — dprint-formatted so the committed snapshot and the gate are formatter-stable. @param {Surface} surface @returns {string} */
const renderSurfaceModule = (surface) => {
  ...
  return formatWithDprint(`${header}${JSON.stringify(surface, null, 2)}\n`, SURFACE_PATH)
}
...
/** Gate 2: the committed matrix vs regeneration. @param {Surface} surface @returns {number} 0 when synced, 1 when drifted */
const checkMatrix = (surface) => {
  const committed = fs.existsSync(MATRIX_PATH) ? fs.readFileSync(MATRIX_PATH, 'utf8') : ''
  let rendered = ''
  try {
    rendered = formatWithDprint(renderMatrix(surface), MATRIX_PATH)
  } catch (error) {
    process.stderr.write(`parity:check FAIL — ${error.message}\n`)
    process.exit(2)
  }
  const diff = diffLines(MATRIX_PATH, committed, rendered)
...
const main = () => {
  ...
  if (mode === 'write') {
    ...
    fs.writeFileSync(MATRIX_PATH, formatWithDprint(renderMatrix(loadSurface(installed)), MATRIX_PATH))
