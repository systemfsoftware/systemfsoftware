...
export const ASSERTED_EXPECTED =
  'a command property holding a schema class the compiler checked, never a value re-labelled to look like one' as const
export const ASSERTED_ACTUAL = 'a type assertion at the command property' as const
...
export const LAUNDERED_EXPECTED =
  'a command property holding a schema class or the Effect subclass call that produces one' as const
export const LAUNDERED_ACTUAL = 'a call at the command property whose callee is not a schema-class member' as const
...
export const DECLARED_EXPECTED = 'a command property holding a schema class that exists at runtime' as const
export const DECLARED_ACTUAL = 'a `declare`d binding at the command property' as const
export const DECLARED_FIX =
  'delete the `declare` and define the schema class, or import the real one; a declared binding produces no value, so this command property is empty at runtime no matter what its type says' as const

export const meta = {
  type: 'problem',
  docs: {
    description:
      'The command property of Workflow.make holds a schema class, not a value laundered into looking like one. Only the positions the compiler cannot decide are reported.',
  },
  schema: [Options],
  messages: {
