...
export const OUTSIDE_EXPECTED =
  'a workflow constructed with Workflow.make only in a <stem>.workflow.ts file whose stem is one segment with no periods' as const
export const OUTSIDE_ACTUAL =
  'a Workflow.make construction in a file that is not a single-segment <stem>.workflow.ts' as const
...
export const SECOND_ACTUAL = 'a second Workflow.make construction in the same file' as const
