...
export const meta = {
  ...
  docs: {
    description:
      'Ban runtime construction outside a module-scope lazy memoized bootstrap closure. Two verdicts, both decided from the call itself: `ManagedRuntime.make`, `Layer.provide`, or `Cell.provide` called inside a function body rebuilds wiring per call, and `ManagedRuntime.make` evaluated at module scope constructs the runtime at import time. Lawful and silent: construction whose result is handed on for reuse - written into a cache binding (an assignment, a `||=`, or a `??=` anywhere between the call and the module-scope binding), or declared by a binding that the closures the exempted factory returns read - inside a module-scope closure, so every call reads the value built once per factory invocation; module-scope `Layer.provide`/`Cell.provide` graph composition; and every `cell.run(input)` arrow application at any depth. Capture is read off the syntax alone: an identifier of the bound name inside any function nested within the exempted closure counts, whether or not that nested function itself later runs once per call, so a same-named shadow inside one counts too and a read inside the construction expression counts too. A module-scope closure that merely wraps the call without handing its result to a nested function hands back a fresh runtime on every call, so it is wiring built per call and reports - as does a binding read only in the body of the enclosure (`const rt = ManagedRuntime.make(AppLive); return rt.run(req)`), which is consumed once per call. Callers resolve through their import specifier (`effect`, `@systemfsoftware/effect-cell-types`), so an aliased import, a namespace import, and a member taken off either one report the same; a module-scope `const` alias of an import is followed, a chain through another module is not, and a tracked call reached through a re-export chain is not visible - the receiver must resolve to a direct import in the same file. Runtime test files (`.test.ts`, `.spec.ts`) are in scope - vitest executes them, so the shapes are unlawful there - but a `.tst.ts` type-test file runs nowhere, so it is out of scope. The eager-construction verdict is exempted only in a module whose exact basename the `edges` option declares - nothing is built in, so `main.ts` and `global-setup.ts` report like any other module until a consumer names its runner-contracted edge in its own config. The exemption covers module-scope composition alone: construction inside a function body still reports in a declared edge.',
  },
  schema: [Options],
  messages: {
