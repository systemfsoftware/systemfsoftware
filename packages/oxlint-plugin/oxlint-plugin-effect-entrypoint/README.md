...
| `runtime-construction-placement` | `ManagedRuntime.make`, `Layer.provide`, or `Cell.provide` called inside a function body — wiring rebuilt per call — and `ManagedRuntime.make` evaluated at module scope. The module-scope lazy bootstrap is lawful only when the construction's result is written into a cache binding (a plain assignment, or a `??=` / `\|\|=` form): a closure that merely wraps the call hands back a fresh runtime on every call and reports. Every `cell.run(input)` arrow application is lawful, and module-scope composition is exempt from the eager-construction verdict only in a module whose exact basename the package declares in the rule's `edges` option — no basename is built in, so `main.ts` and `global-setup.ts` report until their own package declares them, and wiring built inside a function body still reports in a declared edge; a `.tst.ts` type-test file runs nowhere, so it is out of scope |
