...
A: Three of the five rules are filename-gated to the exact basename `main.ts`. `entrypoint-not-imported` and `runtime-construction-placement` run on every file — the violation lives in the importer or in the constructing module; `cell.run(input)` at any depth, module-scope layer composition, module-scope composition in a runner-contracted edge — the exact basenames `main.ts` and `global-setup.ts` — and a clean file all report nothing, so that is the usual answer.

**Q: My app builds its runtime in a module-scope lazy thunk. Does `runtime-construction-placement` report it?**
A: No — that is the lawful shape: `const getRuntime = () => (runtime ??= ManagedRuntime.make(AppLive))`, or the same construction deferred inside a dynamic import. What fails is a runtime built inside any other function body, and `ManagedRuntime.make` evaluated at import time, where importing the module starts fibers no entrypoint can interrupt. One exemption, by exact basename: `main.ts` and a test runner's `global-setup.ts` hook module _are_ the process edge — the runner imports each once per process — so module-scope composition there is the sanctioned shape; a runtime built inside a function body in either still reports.
