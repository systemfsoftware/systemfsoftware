...
| **IO3** | The resolver stays self-contained: it imports nothing from any rule, plugin, or `MakeBoundary` — origin resolution is vocabulary-agnostic (schema/workflow verdicts live in the consumers).                                                                                                                                                                           | `review` — the package's dependency list and the imports in `src/`                    |
