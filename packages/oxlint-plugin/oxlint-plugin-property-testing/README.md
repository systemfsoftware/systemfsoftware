...
> Enforce the property-testing contract: `it.prop` / `it.effect.prop` predicates return a boolean on every path, never call `expect`/`assert`, and live only in `*.property.test.ts` files.
