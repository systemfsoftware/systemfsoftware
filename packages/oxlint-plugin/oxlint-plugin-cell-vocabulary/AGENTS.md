...
Rule vocabulary is read off `Cell.vocabulary` at load, not declared here; not keyed to a cell. Shared conventions: `packages/oxlint-plugin/AGENTS.md`. Root `AGENTS.md` governs.
...
| **CELL-V5** | A `Cell.run` whose success binding feeds a second `Cell.run` inside one `Effect.gen` body is forbidden (`no-sequenced-cell-run`); `Cell.andThen` is the fix. Canonical `Cell`/`Effect` spellings only — aliases stay silent.                                                                      | `pnpm --filter @systemfsoftware/oxlint-plugin-cell-vocabulary test -- src/rules/__tests__/no-sequenced-cell-run.test.ts` exits 0                           |
| **CELL-V6** | A `provideService` piped onto `Cell.run` of a service in that Cell's declared `R` is forbidden (`no-laundered-cell-service`); top-level `Program`-scope provisioning and provides on raw `Effect` values stay silent, unresolvable `R` fails open.                                                | `pnpm --filter @systemfsoftware/oxlint-plugin-cell-vocabulary test -- src/rules/__tests__/no-laundered-cell-service.test.ts` exits 0                       |
