...
| **CELL-V4** | OX-OB1 does not apply — prohibitions only, no obligation rule (the description-requirement obligation is recorded unowned in `docs/solutions/architecture-patterns/make-boundary-owns-a-decision.md`).                                                                                                               | `review` — `configs.recommended` registers prohibitions only                                                                                               |
| **CELL-D1** | Delivered consumer-side through each consuming package's own `extends` of this package's `./preset` — lived instance `effect-daemon-spec`; `@systemfsoftware/oxlint-config/base` omits the fragment because the `effect-cell-types` dependency closes a turbo package-graph cycle through base's consumers (OX-DL1). | `review`                                                                                                                                                   |
