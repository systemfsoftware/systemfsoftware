...
Turned on where a config extends this package's `./preset` fragment — by the transitional `@systemfsoftware/oxlint-config/base` (the nine leaf fragments) and by the strict root `@systemfsoftware/oxlint-preset` (the ten). The id is owned by this package — `@systemfsoftware/oxlint-plugin-effect-native/<rule>` — and a `// oxlint-disable` comment names the short display namespace `@systemfsoftware/effect-native/<rule>`.
