...
Not a plugin. One module (`MakeBoundary.ts`), consumed at build time by the plugins that judge decision bodies: `oxlint-plugin-effect-workflow`. Declared as a devDependency and bundled into each consumer's dist, so published plugins stay standalone with no inter-plugin dependency.
