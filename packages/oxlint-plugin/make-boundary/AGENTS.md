...
| **MB3** | The boundary locator stays workflow-specific: `WORKFLOW_SOURCE`, `WORKFLOW_IMPORT_NAME`, and `WORKFLOW_CONSTRUCTOR_MEMBERS` are declared here — three constants, no dynamic discovery, so a constructor is a member of that set and `MakeBoundary.takesDeciderBody` carries whether it takes a decision body (the stryker-plugins workflow-make-ignorer cannot be imported from the oxlint side); origin verdicts for other vocabularies live in their own consumers. | `review` — the module's imports and the three declared constants                      |
