...
| `kind-file-construction`          | Require a `*.resource.ts` file to mint through `Resource.make` and a `*.handle.ts` file through `Handle.make`, both imported from `@systemfsoftware/effect-cell-types`. |
| `kind-file-declares-no-service`   | Forbid `Context.Service` / `Context.Tag` / `Context.Key` declarations in resource and handle files.                                                                     |
| `kind-file-holds-no-module-state` | Forbid module-level `let`/`var`, mutable collections, Refs, or arrays/objects mutated later in resource and handle files.                                               |
| `kind-record-minted-by-kind`      | Forbid object literals spreading `Pipeable.Prototype` or carrying a computed `[TypeId]` key in resource and handle files.                                               |
...
Included in `@systemfsoftware/oxlint-config-cell-architecture`. The kind rules are registered but not part of `configs.recommended`; they are enrolled per package once its `*.resource.ts` / `*.handle.ts` files are migrated onto `Resource.make` / `Handle.make`.
