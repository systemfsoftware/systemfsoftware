...
| MV-A2 | The contract lane runs only where hardware virtualization exists. Locally without an accessible `/dev/kvm` every suite skips with the reason in its title; in CI (`CI` set) a missing `/dev/kvm` fails the `contract` project from its `globalSetup`, `vitest-kvm-preflight.ts` — it never skips, and the other projects never load it. | `env -u CI pnpm --filter @systemfsoftware/effect-daemon-microvm test:contract` (skips) and the same with `CI=1` (fails without KVM)                |
| MV-A3 | `vitest.config.ts` declares three projects with disjoint includes: `unit`, `conformance` (`tests/**/*.conformance.test.ts`, the port-double runtime, no VM) and `contract` (`tests/**/*.contract.test.ts`, real microVMs). `test` runs `unit` and `conformance`; `test:contract` runs `contract`.                                       | `CI=1 pnpm --filter @systemfsoftware/effect-daemon-microvm test` passes on a host without KVM                                                      |
