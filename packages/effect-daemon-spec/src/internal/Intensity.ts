import { Clock, Duration, Effect, Function, Ref } from 'effect'
...
/** @internal */
export const make: {
  (window: Duration.Duration): (restarts: number) => Effect.Effect<IntensityTracker>
  (restarts: number, window: Duration.Duration): Effect.Effect<IntensityTracker>
} = Function.dual(
  2,
  (restarts: number, window: Duration.Duration): Effect.Effect<IntensityTracker> =>
    Effect.gen(function*() {
