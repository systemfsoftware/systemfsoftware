import { Cause, Duration, Effect, Exit, Match, Metric, Option, Schedule, Scope, Stream } from 'effect'
...
type WorkerShape<_W, E, R> = {
  readonly name: string
  readonly tick: { readonly tickTimeout: Duration.DurationInput; readonly spanName?: string | undefined }
...
const openReadyGauge = (gauge: Metric.Metric.Gauge<number>, name: string) =>
  Metric.set(Metric.tagged(Metric.tagged(gauge, 'daemon', name), 'latch', 'ready'), 1)
...
const buildPollTick = <E, R, W extends WorkerShape<unknown, E, R>>(
  ...
  readyGauge: Metric.Metric.Gauge<number>,
  ...
  return retried.pipe(
    Effect.tap(() => Effect.zipRight(health.ready.open, openReadyGauge(readyGauge, worker.name))),
    Effect.asVoid,
  )
}
...
const buildStreamLoop = <E, R, W extends WorkerShape<unknown, E, R>>(
  ...
  const body = Effect.gen(function*() {
    ...
    const fiber = yield* Effect.forkScoped(
      loop.stream.pipe(
        Stream.tap(() => Effect.zipRight(health.ready.open, openReadyGauge(readyGauge, worker.name))),
        Stream.runDrain,
      ),
    )
