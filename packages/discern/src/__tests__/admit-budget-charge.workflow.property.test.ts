...
it.prop(
  ...
  ([spentDecisions, requestedDecisions]) =>
    tagOf(admitBudgetCharge(new AdmitBudgetCharge({ spentDecisions, spentCalls: 0, requestedDecisions }))) ===
      'ChargeAdmitted',
)
...
it.prop(
  ...
  [AdmitBudgetCharge],
  ([command]) => Result.isFailure(admitBudgetCharge(command)) || withinLimits(command),
)
