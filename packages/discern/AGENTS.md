...
| **D6** | Stateful records (`budget.ts`, `observation-store.ts`) are minimal records: `TypeId`, data, and an opaque `MutableRef`, with no function fields; every operation is a standalone Effect-returning function. Neither acquires nor releases anything, so neither is a `Handle` kind or carries the `*.handle.ts` suffix. Every public method on a builder record has a same-name standalone `dual`. | `review` — `test-types/*.tst.ts` pin each method/dual pair                                                                                                                            |
