      ...
      Gherkin.Do.pipe(
        ...
        Then('the third-party model was asked once, the cache paid no allowance, and the recording replays')((s) =>
          Effect.gen(function*() {
            ...
            expect(s.mount.store.size()).toBe(1)
            expect(s.mount.spend.spent()).toStrictEqual({ decisions: 1, calls: 1 })
            const replayed = yield* Effect.provide(s.policy('x'), Discern.Model.replayLayer(s.mount.store.snapshot()))
