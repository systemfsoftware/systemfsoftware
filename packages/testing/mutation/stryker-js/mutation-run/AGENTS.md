...
- **The manifest's entry list is the API, and a sibling's convenience does not extend it.** Five subpaths existed only because `cli` or `mutation-report` wanted a module — a timer, this package's own version, the plugin barrel, the incremental differ, and a re-export barrel of the failure identities. Each published an internal helper as a name an adopter could import and we could not then move. The rule is now the one thing an adopter can check: a name reaches them through `src/index.ts` or a declared subpath, or it is internal. When a sibling needs an internal, the choice is to make it API deliberately or to give the sibling what it actually needs — `mutation-report` read one number off that timer, so it now takes the number.
