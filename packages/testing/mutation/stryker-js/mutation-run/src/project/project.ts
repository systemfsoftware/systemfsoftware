    ...
    if (this.isEmpty) {
      log.warn(
        normalizeWhitespaces(
          `No files found in directory ${process.cwd()} using ignore rules: ${JSON.stringify(ignoreRules)}. 
      ...
      if (this.filesToMutate.size) {
        const incrementalInfo = this.incrementalReport
          ? ` using incremental report with ${
            ...
            Object.values(this.incrementalReport.testFiles ?? {}).reduce((total, { tests }) => total + tests.length, 0)
          } test(s)${
            force
        ...
        const msg = normalizeWhitespaces(
