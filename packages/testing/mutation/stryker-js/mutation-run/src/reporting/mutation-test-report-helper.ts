...
import { type I, normalizeFileName, normalizeWhitespaces, type requireResolve } from '@systemfsoftware/stryker-js-util'
    ...
    const fileResultsByName = new Map<string, schema.FileResult>(
      await Promise.all(
        [...new Set(results.map(({ fileName }) => fileName))].map(
          async (fileName) => [fileName, await this.toFileResult(fileName)] as const,
        ),
      ),
    )
