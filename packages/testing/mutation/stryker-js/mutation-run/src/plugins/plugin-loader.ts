...
import { isErrnoException, notEmpty, propertyPath } from '@systemfsoftware/stryker-js-util'
...
export class PluginLoader {
  ...
  public load(
    ...
    return Effect.gen({ self: this }, function*() {
      const pluginModules = yield* this.resolvePluginModules(pluginDescriptors)
      const loadedPluginModules = yield* Effect.forEach(
        pluginModules,
        (moduleName: string) =>
          this.loadPlugin(moduleName).pipe(
            Effect.map((plugin) =>
              plugin === undefined
                ? undefined
                : {
      ...
      ).pipe(Effect.map((arr) => arr.filter(notEmpty)))
      ...
      for (const entry of loadedPluginModules) {
        const plugins = (entry as { plugins: readonly PluginContribution<PluginKind>[] | undefined }).plugins
  ...
  private resolvePluginModules(
    ...
    return Effect.gen({ self: this }, function*() {
        ...
        (pluginExpression: string) =>
          Effect.gen({ self: this }, function*() {
            if (pluginExpression.includes('*')) {
              return yield* this.globPluginModules(pluginExpression)
      ...
      return results.filter(notEmpty).flat()
    })
  }
  ...
  private globPluginModules(
    ...
    return Effect.gen({ self: this }, function*() {
      ...
      const pluginNames = yield* this.readOrgDirectory(org)
  ...
  > {
    return Effect.gen({ self: this }, function*() {
      ...
      const moduleEffect = importModule(descriptor).pipe(
        Effect.catch((error: unknown) => {
          if (
          ...
          this.log.warn('Error during loading "%s" plugin', descriptor)
