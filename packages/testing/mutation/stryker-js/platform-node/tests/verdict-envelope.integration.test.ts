  ...
  scenario(
    ...
    Gherkin.Do.pipe(
      ...
      Then('every named field is present and the actionable mutants are listed')((s: {
        ...
        expect(s.envelope.thresholds).toStrictEqual({ high: 80, low: 60, break: 80 })
        expect(s.envelope.counts).toStrictEqual({
          killed: 2,
          timeout: 0,
          survived: 1,
          noCoverage: 1,
        ...
        expect(s.envelope.mutants.length).toBe(2)
        expect(s.envelope.mutants.map((mutant) => mutant.id)).toStrictEqual(['1', '3'])
      }),
    ),
  )
  ...
  scenario(
    ...
    Gherkin.Do.pipe(
      ...
      Then('each mutant entry carries file, location, mutator, replacement and status')((s: {
        ...
        expect(s.envelope.mutants).toStrictEqual([
          {
            id: '1',
            file: 'src/subject.ts',
            location: LOCATION_A,
  ...
  scenario(
    ...
    Gherkin.Do.pipe(
      ...
      Then('there is no score, no report file and no mutants')((s: { envelope: VerdictEnvelope }) => {
        expect(s.envelope.score).toBe(null)
        expect(s.envelope.reportFile).toBe(null)
        expect(s.envelope.mutants).toStrictEqual([])
  ...
  scenario(
    ...
    Gherkin.Do.pipe(
      ...
      Then('the line stays under the 64 KB scanner limit')((s: { line: string }) => {
        expect(s.line.length > 0).toBeTruthy()
        expect(new TextEncoder().encode(s.line).byteLength < 64 * 1024).toBeTruthy()
      }),
    ),
  )
})
