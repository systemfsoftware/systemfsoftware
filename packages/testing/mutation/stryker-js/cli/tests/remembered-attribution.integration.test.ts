...
import { nodePlatformLayer } from './__fixtures__/node-platform.js'
    ...
    scenario(
      ...
      Gherkin.Do.pipe(
        ...
        Then('the killed mutant keeps killedBy and Remembered, and the timeout stays unattributed')((s: {
          ...
          checkExpect(killed).toBeDefined()
          checkExpect(killed?.killedBy).toEqual(['t1'])
          checkExpect(killed?.coveredBy).toEqual(['t1'])
          checkExpect(killed?.statusReason).toBe('Remembered')
          checkExpect(timedOut).toBeDefined()
          checkExpect(timedOut?.killedBy).toBeUndefined()
          checkExpect(timedOut?.statusReason).toBe('Remembered')
        }),
      ),
    )
  })
