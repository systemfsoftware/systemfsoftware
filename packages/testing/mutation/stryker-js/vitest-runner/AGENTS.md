...
🛑 `src/Runner.ts` resolves `stryker-setup.mjs` next to its own emitted module and copies it into the sandbox (as `stryker-setup-<pid>.js`, prepended to each project's setup files), so the integration specs need a fresh `dist/`. `turbo.json` in this package makes `test` depend on its own `build`, so run `pnpm turbo test --filter=@systemfsoftware/stryker-js-vitest-runner`; a bare `pnpm --filter … test` tests the previous build.
