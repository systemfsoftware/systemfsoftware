...
| **VR2** | This adapter currently has no test net: the in-package integration suite was deleted (it could only reach the runner through `src/`, and no surviving suite loads the plugin — the cli integration tests pin `testRunner: 'command'`), so `Runner.ts`, `Runner.schema.ts`, and the hit-limit path in `interpret-vitest-run.workflow.ts` have zero coverage. Coverage returns via a published runner entry or decisions behind the published plugin surface — never widen `exports` for a test (KTD3). | `review`                                                            |
