...
- **Printer changes ride the instrument characterization suite.** The printed output is exercised through the public `instrument` entry (`tests/instrumenter.integration.test.ts` — comments and hashbang survival among the scenarios); the printer has no direct unit net of its own, so a printer change that the characterization suite cannot observe ships unreviewed. Gate: `pnpm --filter @systemfsoftware/stryker-js-instrumenter test`.
