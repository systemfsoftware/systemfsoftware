  ...
  Effect.gen(function*() {
    ...
    const getFile = (fileName: string): Effect.Effect<ScriptFile | undefined, never> =>
      Effect.gen(function*() {
        ...
        const content = yield* fsService.readFileString(normalized).pipe(
    ...
    const setTsConfigOverrides = (overrides: Map<string, string>): Effect.Effect<void> =>
      Ref.set(overridesRef, overrides)
