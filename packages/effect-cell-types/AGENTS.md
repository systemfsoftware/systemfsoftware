...
Contracts for the cell types other packages build against: `Workflow<Command, Decision, Error>` with its `make`, the `Cell` spine (`Cell.layer`, the combinator arrows, arrow application `cell.run(input)`). Root `AGENTS.md` governs.

## Rules

| ID          | Rule                                                                                                                                                                                                                                                                                                                                                                                                            | Gate                                                                                                                             |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **CELL-T1** | The runtime stays limited to `Cell.layer`, the eight arrows (`map`, `mapInput`, `andThen`, `zip`, `gate`, `collect`, `collectAll`, `provide`), arrow application (`cell.run(input)`), and `layerRunner`. No `stryker.config.json`, no mutation script, no phase machinery in the public surface — this package holds types plus one interpreter, so a mutation score here reports on someone else's test suite. | `git ls-files packages/effect-cell-types` names no `stryker.config.json`; `package.json` carries no mutation script              |
