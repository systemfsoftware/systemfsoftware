...
/**
 * Service for the active {@link Frame}.
 */
...
export const buildFrame: {
  (frame: CoreFrame, wrap: Wrappers): Frame
  (wrap: Wrappers): (frame: CoreFrame) => Frame
} = dual(2, (frame: CoreFrame, wrap: Wrappers): Frame => {
  ...
  return Frame.of({
    ...
    evaluate: <R, Arg = void>(
      f: PageEvaluateFunction<Arg, R>,
      arg?: Arg,
      options?: Parameters<CoreFrame['evaluate']>[2],
    ) =>
      use((frame) => {
        assertEvaluateArg<Arg>(arg)
        return frame.evaluate<R, Arg>(f, arg, options)
      }),
    title: use((f) => f.title()),
    use,
    locator: (selector, options) => wrap.locator(frame.locator(selector, options)),
    getByRole: (role, options) => wrap.locator(frame.getByRole(role, options)),
    getByText: (text, options) => wrap.locator(frame.getByText(text, options)),
    getByLabel: (label, options) => wrap.locator(frame.getByLabel(label, options)),
    getByTestId: (testId) => wrap.locator(frame.getByTestId(testId)),
    getByPlaceholder: (text, options) => wrap.locator(frame.getByPlaceholder(text, options)),
    getByAltText: (text, options) => wrap.locator(frame.getByAltText(text, options)),
    getByTitle: (text, options) => wrap.locator(frame.getByTitle(text, options)),
    page: () => wrap.page(frame.page()),
    parentFrame: () => Option.fromNullishOr(frame.parentFrame()).pipe(Option.map(wrap.frame)),
    childFrames: () => Array.map(frame.childFrames(), (f) => wrap.frame(f)),
