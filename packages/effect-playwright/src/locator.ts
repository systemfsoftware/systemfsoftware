...
import type { FrameLocator } from './frame-locator.js'
import type { Page } from './page.js'
import { useHelper } from './utils.js'
import type { Wrappers } from './wrappers.js'

declare const coreLocatorEvaluate: CoreLocator['evaluate']

/**
 * The page function argument accepted by {@link Locator.evaluate} and
 * {@link Locator.evaluateHandle}.
 */
export type LocatorEvaluateFunction<
  R,
  Arg,
  E extends SVGElement | HTMLElement = SVGElement | HTMLElement,
> = Parameters<typeof coreLocatorEvaluate<R, Arg, E>>[0]

declare const coreLocatorEvaluateAll: CoreLocator['evaluateAll']

/**
 * The page function argument accepted by {@link Locator.evaluateAll}.
 */
export type LocatorEvaluateAllFunction<
  R,
  Arg,
  E extends SVGElement | HTMLElement = SVGElement | HTMLElement,
> = Parameters<typeof coreLocatorEvaluateAll<R, Arg, E>>[0]
...
export interface Locator {
   ...
   */
  readonly click: (options?: Parameters<CoreLocator['click']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
   * Checks the element.
   *
   * @see {@link CoreLocator.check}
   */
  readonly check: (options?: Parameters<CoreLocator['check']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
   * Fills the input field.
   *
   * @see {@link CoreLocator.fill}
   */
  readonly fill: (value: string, options?: Parameters<CoreLocator['fill']>[1]) => Effect.Effect<void, PlaywrightError>
  /**
   ...
   */
  readonly innerText: (options?: Parameters<CoreLocator['innerText']>[0]) => Effect.Effect<string, PlaywrightError>
  /**
   * Gets the inner HTML.
   *
   * @see {@link CoreLocator.innerHTML}
   */
  readonly innerHTML: (options?: Parameters<CoreLocator['innerHTML']>[0]) => Effect.Effect<string, PlaywrightError>
  /**
   ...
   */
  readonly boundingBox: (options?: Parameters<CoreLocator['boundingBox']>[0]) => Effect.Effect<
   ...
   */
  readonly getByTestId: (testId: Parameters<CoreLocator['getByTestId']>[0]) => Locator
  /**
   ...
   */
  readonly waitFor: (options?: Parameters<CoreLocator['waitFor']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
  ...
  readonly waitForFunction: <
    ...
    E extends SVGElement | HTMLElement = SVGElement | HTMLElement,
  >(
    pageFunction: LocatorEvaluateFunction<R, Arg, E>,
    arg?: Arg,
  ...
  readonly evaluateAll: <
    ...
    E extends SVGElement | HTMLElement = SVGElement | HTMLElement,
  >(
    pageFunction: LocatorEvaluateAllFunction<R, Arg, E>,
    arg?: Arg,
  ) => Effect.Effect<R, PlaywrightError>
  /**
  ...
  ) => Effect.Effect<Option.Option<ElementHandle<SVGElement | HTMLElement>>, PlaywrightError>
  /**
   * Resolves given locator to all matching DOM elements.
   *
   * @see {@link CoreLocator.elementHandles}
   */
  readonly elementHandles: Effect.Effect<ReadonlyArray<ElementHandle>, PlaywrightError>
  /**
   ...
   */
  readonly blur: (options?: Parameters<CoreLocator['blur']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
   * Clear the input field.
   *
   * @see {@link CoreLocator.clear}
   */
  readonly clear: (options?: Parameters<CoreLocator['clear']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
   ...
   */
  readonly focus: (options?: Parameters<CoreLocator['focus']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
   * Hovers over the element.
   *
   * @see {@link CoreLocator.hover}
   */
  readonly hover: (options?: Parameters<CoreLocator['hover']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
   ...
   */
  readonly tap: (options?: Parameters<CoreLocator['tap']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
   * Unchecks the element.
   *
   * @see {@link CoreLocator.uncheck}
   */
  readonly uncheck: (options?: Parameters<CoreLocator['uncheck']>[0]) => Effect.Effect<void, PlaywrightError>
  /**
   ...
   */
  readonly use: <T>(f: (locator: CoreLocator) => Promise<T>) => Effect.Effect<T, PlaywrightError>
}

/**
 * A service that provides a `Locator` instance.
 */
export const Locator = Context.Service<Locator>('effect-playwright/locator/Locator')
...
export const buildLocator: {
  (locator: CoreLocator, wrap: Wrappers): Locator
  (wrap: Wrappers): (locator: CoreLocator) => Locator
} = dual(2, (locator: CoreLocator, wrap: Wrappers): Locator => {
  ...
  return Locator.of({
    ...
    boundingBox: (options) => use((locator) => locator.boundingBox(options)).pipe(Effect.map(Option.fromNullishOr)),
    describe: (description) => wrap.locator(locator.describe(description)),
    description: () => Option.fromNullishOr(locator.description()),
    count: use((locator) => locator.count()),
    first: () => wrap.locator(locator.first()),
    last: () => wrap.locator(locator.last()),
    nth: (index: number) => wrap.locator(locator.nth(index)),
    all: use((locator) => locator.all()).pipe(Effect.map(Array.map(wrap.locator))),
    and: (locatorOrService) => wrap.locator(locator.and(unwrap(locatorOrService))),
    contentFrame: () => wrap.frameLocator(locator.contentFrame()),
    filter: (options) => wrap.locator(locator.filter(options)),
    frameLocator: (selector) => wrap.frameLocator(locator.frameLocator(selector)),
    or: (locatorOrService) => wrap.locator(locator.or(unwrap(locatorOrService))),
    page: () => wrap.page(locator.page()),
    ...
    getByRole: (role, options) => wrap.locator(locator.getByRole(role, options)),
    getByText: (text, options) => wrap.locator(locator.getByText(text, options)),
    getByLabel: (text, options) => wrap.locator(locator.getByLabel(text, options)),
    getByPlaceholder: (text, options) => wrap.locator(locator.getByPlaceholder(text, options)),
    getByAltText: (text, options) => wrap.locator(locator.getByAltText(text, options)),
    getByTitle: (text, options) => wrap.locator(locator.getByTitle(text, options)),
    getByTestId: (testId) => wrap.locator(locator.getByTestId(testId)),
    ...
    >(
      ...
      arg?: Arg,
      options?: Parameters<CoreLocator['waitForFunction']>[2],
    ) => use((locator) => locator.waitForFunction<Arg, E>(pageFunction, arg, options)),
    evaluate: <
      R,
      Arg = void,
      E extends SVGElement | HTMLElement = SVGElement | HTMLElement,
    >(
      ...
      arg?: Arg,
      options?: Parameters<CoreLocator['evaluate']>[2],
    ) => use((locator) => locator.evaluate<R, Arg, E>(pageFunction, arg, options)),
    evaluateAll: <
      R,
      Arg = void,
      E extends SVGElement | HTMLElement = SVGElement | HTMLElement,
    >(
      pageFunction: LocatorEvaluateAllFunction<R, Arg, E>,
      arg?: Arg,
    ) => use((locator) => locator.evaluateAll<R, Arg, E>(pageFunction, arg)),
    evaluateHandle: <
      R,
      Arg = void,
      E extends SVGElement | HTMLElement = SVGElement | HTMLElement,
    >(
      ...
      arg?: Arg,
      options?: Parameters<CoreLocator['evaluateHandle']>[2],
    ) => use((locator) => locator.evaluateHandle<R, Arg, E>(pageFunction, arg, options)),
    elementHandle: (options) => use((locator) => locator.elementHandle(options)).pipe(Effect.map(Option.fromNullishOr)),
    elementHandles: use((locator) => locator.elementHandles()),
    ...
    normalize: use((locator) => locator.normalize()).pipe(Effect.map(wrap.locator)),
