...
const answered = (state: StoreState, trace: string): TraceResponse => {
  ...
  if (held.growing) return { _tag: 'Unfinished', traceId: trace }
...
const stepped = (state: StoreState, command: TraceCommand): Step =>
  Match.value(command).pipe(
    Match.tagsExhaustive({
      Serve: (serve): Step => [withHeld(state, { trace: serve.trace, spans: serve.spans, growing: false }), done],
      Grow: (grow): Step => [withHeld(state, { trace: grow.trace, spans: [], growing: true }), done],
